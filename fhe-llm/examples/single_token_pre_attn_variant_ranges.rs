...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for row in pipeline.diagnose_pre_attn_norm_variants_for_token(token_id) {
        println!(
            "variant={} decode@{} min={} max={} overflow_dims={}/{}",
            row.variant, row.decode_precision, row.min, row.max, row.overflow_dims, row.total_dims
        );
    }
