...
//! FHE_LLM's target workload (20B-40B parameter transformers with 32+
...
//! - [`FHE_LLMBootstrapKey`]: key material for the bootstrapping pipeline
//! - [`FHE_LLMBootstrapKeyPrepared`]: DFT-prepared key material for fast evaluation
//! - [`needs_bootstrap`]: checks whether a ciphertext's estimated noise
//!   budget requires refreshing before further computation
//! - [`_FHE_LLM_bootstrap`]: bootstraps a single GLWE ciphertext coefficient
//!
//! ## Bootstrapping Pipeline
//!
//! The bootstrapping pipeline follows the standard TFHE pattern adapted to
//! FHE_LLM's bivariate torus representation:
//!
...
//! ## Design Rationale
//!
//! FHE_LLM's design prioritises **bootstrapping elimination** for shallow
...
//!   [`FHE_LLMParams::max_depth`]
//! - Models deeper than [`FHE_LLMParams::max_layers_no_bootstrap`] layers
...
pub struct BootstrappingConfig {
    ...
    /// [`_FHE_LLM_bootstrap`] will panic. Use this for models known to
...
/// * `params` - FHE_LLM parameter set (for budget calculation).
/// * `config` - Bootstrapping configuration.
pub fn needs_bootstrap(tracker: &NoiseTracker, params: &FHE_LLMParams, config: &BootstrappingConfig) -> bool {
    if !config.enabled {
        return false;
    }
...
pub fn estimate_bootstrap_cost(params: &FHE_LLMParams) -> BootstrapCostEstimate {
...
///
/// These are derived from the main [`FHE_LLMParams`] and control the
...
#[derive(Clone, Debug)]
pub struct FHE_LLMBootstrapParams {
...
impl FHE_LLMBootstrapParams {
    /// Derives bootstrapping parameters from the main FHE_LLM parameter set.
    ///
    ...
    pub fn from_FHE_LLM(params: &FHE_LLMParams) -> Self {
        ...
        // 2^{-(log_message_modulus+1)} on the torus. FHE_LLM encodes at
...
/// Key material for FHE_LLM bootstrapping.
///
...
/// ## Key Domains
///
/// FHE_LLM uses a **ternary** GLWE secret key (rank 1, dimension N), but the
...
pub struct FHE_LLMBootstrapKey<BE: Backend> {
    /// Bootstrapping-specific parameters.
    pub bootstrap_params: FHE_LLMBootstrapParams,
...
impl<BE: Backend> FHE_LLMBootstrapKey<BE>
where
    ...
    Scratch<BE>: ScratchTakeCore<BE>,
{
    ...
    pub fn generate(
        ...
        let bp = FHE_LLMBootstrapParams::from_FHE_LLM(params);
...
/// [`FHE_LLMBootstrapKey`] via [`FHE_LLMBootstrapKeyPrepared::prepare`].
pub struct FHE_LLMBootstrapKeyPrepared<BE: Backend> {
    /// Bootstrapping-specific parameters.
    pub bootstrap_params: FHE_LLMBootstrapParams,
...
impl<BE: Backend> FHE_LLMBootstrapKeyPrepared<BE>
where
    ...
    Scratch<BE>: ScratchTakeCore<BE> + ScratchAvailable,
{
    ...
    pub fn prepare(module: &Module<BE>, bsk: &FHE_LLMBootstrapKey<BE>) -> Self {
...
/// Bootstraps a FHE_LLM ciphertext to refresh its noise budget.
///
...
pub fn _FHE_LLM_bootstrap<BE: Backend>(
    module: &Module<BE>,
    ct: &GLWE<Vec<u8>>,
...
pub fn _FHE_LLM_bootstrap_with_lut<BE: Backend>(
    module: &Module<BE>,
    ct: &GLWE<Vec<u8>>,
    tracker: &mut NoiseTracker,
    bsk_prepared: &FHE_LLMBootstrapKeyPrepared<BE>,
    ...
    Scratch<BE>: ScratchTakeCore<BE> + ScratchAvailable,
{
    ...
    assert_eq!(
        lut_entries.len(),
        message_modulus,
        "_FHE_LLM_bootstrap_with_lut: lut length {} must equal message modulus {}",
        lut_entries.len(),
        message_modulus
    );
...
/// Bootstrap with custom precision (custom log_message_modulus).
///
/// Similar to `_FHE_LLM_bootstrap_with_lut`, but allows specifying a custom
/// `log_message_modulus` for the LUT, enabling higher precision bootstrapping
/// than the default `bp.log_message_modulus`.
pub fn _FHE_LLM_bootstrap_with_lut_custom_precision<BE: Backend>(
    module: &Module<BE>,
    ct: &GLWE<Vec<u8>>,
    tracker: &mut NoiseTracker,
    bsk_prepared: &FHE_LLMBootstrapKeyPrepared<BE>,
    lut_entries: &[i64],
    custom_log_message_modulus: usize,
) -> GLWE<Vec<u8>>
where
    Module<BE>: ModuleN + LWESampleExtract + LWEKeySwitch<BE> + BlindRotationExecute<CGGI, BE> + LookupTableFactory,
    ScratchOwned<BE>: ScratchOwnedAlloc<BE> + ScratchOwnedBorrow<BE>,
    Scratch<BE>: ScratchTakeCore<BE> + ScratchAvailable,
{
    ...
    let message_modulus: usize = 1 << custom_log_message_modulus;
    assert_eq!(
        lut_entries.len(),
        message_modulus,
        "_FHE_LLM_bootstrap_with_lut_custom_precision: lut length {} must equal message modulus {}",
        lut_entries.len(),
        message_modulus
    );
    ...
    let mut lut = LookupTable::alloc(&lut_layout);
    // Use custom precision for LUT encoding
    lut.set(module, lut_entries, custom_log_message_modulus + 1);
...
mod tests {
    ...
    fn test_bootstrap_cost_scales_with_n() {
        let params_80 = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let params_128 = FHE_LLMParams::new(SecurityLevel::Bits128, Precision::Int8);
    ...
    fn test_bootstrap_params_fp16() {
        let params = FHE_LLMParams::new(SecurityLevel::Bits128, Precision::Fp16);
        let bp = FHE_LLMBootstrapParams::from_FHE_LLM(&params);
    ...
    #[test]
    fn test__FHE_LLM_bootstrap_roundtrip() {
    ...
    /// native LWE encoding (bypassing FHE_LLM encoding) to isolate whether
    ...
    fn test_bootstrap_native_lwe_encoding() {
        ...
        // --- Generate keys exactly like FHE_LLMBootstrapKey::generate ---
