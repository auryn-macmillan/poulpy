...
/// Model truncated to d_model=32 for tractability and frequent bootstraps after each major op.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Use TinyLlama model, truncate to d_model=32 and d_ffn=128.
    let config = InferenceConfig {
        ...
        fhe_frequent_bootstrap: true, // bootstrap after QKV, attention, FFN, etc.
        ..InferenceConfig::default()
    };

    let load_start = Instant::now();
    eprintln!("[test] Loading pipeline with FP16 precision and frequent bootstraps...");
