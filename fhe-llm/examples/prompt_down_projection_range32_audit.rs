...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let midrange_result = pipeline.diagnose_prompt_first_layer_down_projection_with_pre_ffn_config(
        &prompt,
        attn_decode,
        residual_decode,
        midrange,
    )?;
    let range32_result = pipeline.diagnose_prompt_first_layer_down_projection_with_pre_ffn_config(
        &prompt,
        attn_decode,
        residual_decode,
        range32,
