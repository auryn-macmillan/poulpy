...
fn parse_shifts(arg: Option<String>) -> Vec<usize> {
    ...
    .unwrap_or_else(|| vec![0, 2, 4, 6, 8, 10, 12])
}
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let sweep = pipeline.sweep_prompt_fhe_score_shift(&prompt, &shifts)?;
    ...
    for row in sweep.rows {
        println!(
            "shift={}: token={} {:?} | L-inf={:.3} L2={:.3} MAE={:.3} | plaintext_rank={} pred_logit={:.4} plaintext_logit={:.4}",
            row.extra_shift_bits,
            row.token_id,
            row.token_text,
            row.linf,
            row.l2,
