...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let midrange_result = pipeline.compare_prompt_fhe_first_layer_stages_against_fhe_like_with_pre_ffn_config(
        &prompt,
        attn_decode,
        residual_decode,
        midrange,
    )?;
    let range32_result = pipeline.compare_prompt_fhe_first_layer_stages_against_fhe_like_with_pre_ffn_config(
        &prompt,
        attn_decode,
        residual_decode,
        range32,
