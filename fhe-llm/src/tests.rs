//! Integration tests for the FHE_LLM scheme.
...
mod integration {
    ...
    fn test_all_security_levels() {
        for security in [SecurityLevel::Bits80, SecurityLevel::Bits100, SecurityLevel::Bits128] {
            for precision in [Precision::Int8, Precision::Fp16] {
                let params = FHE_LLMParams::new(security, precision);
    ...
    fn test_encrypt_decrypt_zeros() {
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [0u8; 32]);

        let values: Vec<i8> = vec![0; 64];
        let pt = encode_int8(&module, &params, &values);
        let ct = _FHE_LLM_encrypt(&module, &key, &pt, [1u8; 32], [2u8; 32]);
        let pt_dec = _FHE_LLM_decrypt(&module, &key, &ct, &params);
    ...
    fn test__FHE_LLM_ct_mul_basic() {
        ...
        // Now test via _FHE_LLM_ct_mul wrapper
        use crate::activations::_FHE_LLM_ct_mul;
        ...
        let ct_prod = _FHE_LLM_ct_mul(&module, &eval_key, &ct_a2, &ct_b2);
        ...
        assert!(
            diff2 <= 2,
            "_FHE_LLM_ct_mul error at coeff 0: have={}, want={}, diff={}",
            decoded2[0],
            decoded_want[0],
            diff2
        );
    ...
    #[test]
    fn test__FHE_LLM_eval_key_generation() {
        // Test that FHE_LLMEvalKey can be generated and has sensible parameters.
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [42u8; 32]);

        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [50u8; 32], [60u8; 32]);
    ...
    /// FHE_LLMKey::generate + FHE_LLMEvalKey::generate + _FHE_LLM_encrypt
    /// + _FHE_LLM_add + _FHE_LLM_decrypt + decode.
    ///
    ...
    fn test_full_pipeline_encrypt_add_decrypt() {
        use crate::arithmetic::_FHE_LLM_add;
        ...
        let _eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [50u8; 32], [60u8; 32]);
        ...
        let ct_a = _FHE_LLM_encrypt(&module, &key, &pt_a, [70u8; 32], [80u8; 32]);
        let ct_b = _FHE_LLM_encrypt(&module, &key, &pt_b, [90u8; 32], [100u8; 32]);

        // Homomorphic addition
        let ct_sum = _FHE_LLM_add(&module, &ct_a, &ct_b);

        // Decrypt and decode using high-level API
        let pt_dec = _FHE_LLM_decrypt(&module, &key, &ct_sum, &params);
    ...
    /// FHE_LLMEvalKey::generate + _FHE_LLM_ct_mul.
    ///
    ...
    fn test_full_pipeline_eval_key_ct_mul() {
        ...
        // Encrypt using _FHE_LLM_encrypt
        let ct = _FHE_LLM_encrypt(&module, &key, &pt, [70u8; 32], [80u8; 32]);

        // ct * ct multiplication using eval key
        let ct_prod = _FHE_LLM_ct_mul(&module, &eval_key, &ct, &ct);
    ...
    fn test_apply_poly_activation_squared_relu() {
        ...
        //
        // Uses the same low-level parameter setup as test__FHE_LLM_ct_mul_basic.
    ...
    fn test_apply_poly_activation_exp_constant_term() {
        ...
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [55u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [56u8; 32], [57u8; 32]);
        ...
        let ct = _FHE_LLM_encrypt(&module, &key, &pt, [58u8; 32], [59u8; 32]);
        let ct_result = apply_poly_activation(&module, &eval_key, &ct, &exp_poly_approx());
        let pt_dec = _FHE_LLM_decrypt(&module, &key, &ct_result, &params);
    ...
    fn test_rescale_single() {
        use crate::arithmetic::{_FHE_LLM_mul_const, _FHE_LLM_rescale};
        ...
        let ct_mul = _FHE_LLM_mul_const(&module, &ct, &[3i64]);

        // Rescale (drops lowest limb)
        let ct_rescaled = _FHE_LLM_rescale(&module, &ct_mul, &params);

        // Decrypt and decode
        let pt_dec = _FHE_LLM_decrypt(&module, &key, &ct_rescaled, &params);
    ...
    fn test_rescale_chained_two_levels() {
        ...
        let ct_mul1 = _FHE_LLM_mul_const(&module, &ct, &[2i64]);
        let ct_rescaled1 = _FHE_LLM_rescale(&module, &ct_mul1, &params);

        // Level 2: multiply by 3, then rescale
        let ct_mul2 = _FHE_LLM_mul_const(&module, &ct_rescaled1, &[3i64]);
        let ct_rescaled2 = _FHE_LLM_rescale(&module, &ct_mul2, &params);

        // Decrypt and decode
        let pt_dec = _FHE_LLM_decrypt(&module, &key, &ct_rescaled2, &params);
        let decoded = decode_int8(&module, &params, &pt_dec, 3);
    ...
    fn test_rescale_precision_tracking() {
        ...
        // NOT params.base2k. _FHE_LLM_rescale drops base2k bits based on
        // the ciphertext's own base2k value.
        let ct_base2k = ct.base2k().as_u32();

        // First mul_const + rescale
        let ct_mul1 = _FHE_LLM_mul_const(&module, &ct, &[2i64]);
        let ct_r1 = _FHE_LLM_rescale(&module, &ct_mul1, &params);
        ...
        let ct_mul2 = _FHE_LLM_mul_const(&module, &ct_r1, &[2i64]);
        let ct_r2 = _FHE_LLM_rescale(&module, &ct_mul2, &params);
    ...
    fn test_rescale_after_add() {
        use crate::arithmetic::{_FHE_LLM_add, _FHE_LLM_mul_const, _FHE_LLM_rescale};
        ...
        let pt_a = encode_int8(&module, &params, &a_vals);
        let pt_b = encode_int8(&module, &params, &b_vals);
        let ct_a = _FHE_LLM_encrypt(&module, &key, &pt_a, [1u8; 32], [2u8; 32]);
        let ct_b = _FHE_LLM_encrypt(&module, &key, &pt_b, [3u8; 32], [4u8; 32]);

        // Add, then multiply by 2, then rescale
        let ct_sum = _FHE_LLM_add(&module, &ct_a, &ct_b);
        let ct_mul = _FHE_LLM_mul_const(&module, &ct_sum, &[2i64]);
        let ct_rescaled = _FHE_LLM_rescale(&module, &ct_mul, &params);
    ...
    fn test_rescale_saturation() {
        ...
        let mut current = _FHE_LLM_mul_const(&module, &ct, &[1i64]);
        let mut prev_k = current.k().as_u32();

        for _ in 0..20 {
            let rescaled = _FHE_LLM_rescale(&module, &current, &params);
            ...
            prev_k = new_k;
            current = _FHE_LLM_mul_const(&module, &rescaled, &[1i64]);
        }
    }
    ...
    fn test_rescale_noise_tracker_consistency() {
        ...
        for round in 0..3 {
            current_ct = _FHE_LLM_mul_const(&module, &current_ct, &[2i64]);
            tracker.mul_const(2.0);

            current_ct = _FHE_LLM_rescale(&module, &current_ct, &params);
            tracker.rescale(params.base2k.0);
            ...
            if budget > 2.0 {
                let pt_dec = _FHE_LLM_decrypt(&module, &key, &current_ct, &params);
    ...
    /// 2. ct_sq from _FHE_LLM_ct_mul decodes correctly at the encoding scale.
    #[test]
    ...
    fn test_end_to_end_transformer_block() {
        ...
        let ct = _FHE_LLM_encrypt(&module, &key, &pt, [50u8; 32], [51u8; 32]);

        // Run the full transformer block
        let result = _FHE_LLM_transformer_block(&module, &eval_key, &ct, &config, &weights);
    ...
    fn test_end_to_end_forward_pass_2_layers() {
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [60u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [61u8; 32], [62u8; 32]);
        ...
        let pt = encode_int8(&module, &params, &vals);
        let ct = _FHE_LLM_encrypt(&module, &key, &pt, [70u8; 32], [71u8; 32]);

        // Run the 2-layer forward pass
        let result = _FHE_LLM_forward_pass(&module, &eval_key, &ct, &config, &layer_weights);
    ...
    fn test_end_to_end_transformer_block_d4() {
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [100u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [101u8; 32], [102u8; 32]);
        ...
        let pt = encode_int8(&module, &params, &vals);
        let ct = _FHE_LLM_encrypt(&module, &key, &pt, [110u8; 32], [111u8; 32]);
    ...
    /// Tests the multi-head attention path in `_FHE_LLM_transformer_block`.
    ///
    ...
    fn test_multi_head_attention_d4_h2() {
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [130u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [131u8; 32], [132u8; 32]);
        ...
        let vals: Vec<i8> = vec![1, 2, 3, 4];
        let pt = encode_int8(&module, &params, &vals);
        let ct = _FHE_LLM_encrypt(&module, &key, &pt, [133u8; 32], [134u8; 32]);
    ...
    #[test]
    fn test_matmul_d4() {
        use crate::arithmetic::_FHE_LLM_matmul_single_ct;

        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [120u8; 32]);

        // Encrypt [1, 0, 0, 0] — a monomial
        let vals: Vec<i8> = vec![1, 0, 0, 0];
        let pt = encode_int8(&module, &params, &vals);
        let ct = _FHE_LLM_encrypt(&module, &key, &pt, [121u8; 32], [122u8; 32]);

        // Weight rows: [[1,0,0,0], [0,1,0,0]] — identity-like rows
        let weight_rows = vec![vec![1i64, 0, 0, 0], vec![0i64, 1, 0, 0]];

        let result = _FHE_LLM_matmul_single_ct(&module, &ct, &weight_rows);
        ...
        let pt_out = _FHE_LLM_decrypt(&module, &key, &result[0], &params);
    ...
    fn test_ffn_standard_d2() {
        ...
        let result = _FHE_LLM_ffn_standard(&module, &eval_key, &ct, &weights, &ActivationChoice::SquaredReLU);
    ...
    fn test_ffn_swiglu_d2() {
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [140u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [141u8; 32], [142u8; 32]);

        // Encrypt [4, 5]
        let vals: Vec<i8> = vec![4, 5];
        let pt = encode_int8(&module, &params, &vals);
        let ct = _FHE_LLM_encrypt(&module, &key, &pt, [143u8; 32], [144u8; 32]);
        ...
        let result = _FHE_LLM_ffn_swiglu(&module, &eval_key, &ct, &weights);
    ...
    fn test_transformer_block_with_gamma() {
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [200u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [201u8; 32], [202u8; 32]);
        ...
        // Gamma = 2.0 in FHE_LLM's fixed-point encoding (2 * 2^8 = 512)
        ...
        let vals: Vec<i8> = vec![5];
        let pt = encode_int8(&module, &params, &vals);
        let ct = _FHE_LLM_encrypt(&module, &key, &pt, [210u8; 32], [211u8; 32]);

        // Run without gamma
        let result_no_gamma = _FHE_LLM_transformer_block(&module, &eval_key, &ct, &config, &base_weights);

        // Run with gamma
        let result_with_gamma = _FHE_LLM_transformer_block(&module, &eval_key, &ct, &config, &gamma_weights);
        ...
        let pt_no = _FHE_LLM_decrypt(&module, &key, &result_no_gamma, &params);
        let pt_yes = _FHE_LLM_decrypt(&module, &key, &result_with_gamma, &params);
        ...
        // `with_gamma()` builder inside `_FHE_LLM_transformer_block()`.
        let _ = (dec_no, dec_yes);
    }

    /// Tests the bootstrapping-aware forward pass with bootstrapping disabled.
    /// This exercises the `_FHE_LLM_forward_pass_with_bootstrap` code path while
    ...
    fn test_forward_pass_with_bootstrap_disabled() {
        ...
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [220u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [221u8; 32], [222u8; 32]);
        ...
        let vals: Vec<i8> = vec![5];
        let pt = encode_int8(&module, &params, &vals);
        let ct = _FHE_LLM_encrypt(&module, &key, &pt, [230u8; 32], [231u8; 32]);

        let bootstrap_cfg = BootstrappingConfig::no_bootstrap();
        let (result, tracker) = _FHE_LLM_forward_pass_with_bootstrap(
            &module,
            &eval_key,
            &ct,
            &config,
    ...
    fn test_forward_pass_with_bootstrap_enabled() {
        ...
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [240u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [241u8; 32], [242u8; 32]);
        ...
        let bsk_prepared = FHE_LLMBootstrapKeyPrepared::prepare(&module, &bsk);
        ...
        let vals: Vec<i8> = vec![3];
        let pt = encode_int8(&module, &params, &vals);
        let ct = _FHE_LLM_encrypt(&module, &key, &pt, [250u8; 32], [251u8; 32]);
    ...
    fn test_accuracy_encrypt_decrypt_baseline() {
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [250u8; 32]);

        let test_vals: Vec<i8> = vec![0, 1, -1, 10, -10, 42, -42, 100, -100, 127, -128];

        let pt = encode_int8(&module, &params, &test_vals);
        let ct = _FHE_LLM_encrypt(&module, &key, &pt, [251u8; 32], [252u8; 32]);
        let pt_dec = _FHE_LLM_decrypt(&module, &key, &ct, &params);
    ...
    fn test_accuracy_addition() {
        ...
        let pt_a = encode_int8(&module, &params, &a_vals);
        let pt_b = encode_int8(&module, &params, &b_vals);
        let ct_a = _FHE_LLM_encrypt(&module, &key, &pt_a, [201u8; 32], [202u8; 32]);
        let ct_b = _FHE_LLM_encrypt(&module, &key, &pt_b, [203u8; 32], [204u8; 32]);
    ...
    fn test_accuracy_mul_const() {
        use crate::arithmetic::_FHE_LLM_mul_const;

        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [210u8; 32]);

        // Encrypt [3, 0, 0, 0]
        let vals: Vec<i8> = vec![3, 0, 0, 0];
        let pt = encode_int8(&module, &params, &vals);
        let ct = _FHE_LLM_encrypt(&module, &key, &pt, [211u8; 32], [212u8; 32]);

        // Multiply by constant polynomial [2, 1, 0, 0] (= 2 + X)
        let constants = vec![2i64, 1, 0, 0];
        let ct_mul = _FHE_LLM_mul_const(&module, &ct, &constants);

        let pt_dec = _FHE_LLM_decrypt(&module, &key, &ct_mul, &params);
    ...
    ///
    /// Matmul is implemented as multiple `_FHE_LLM_mul_const` calls, one per
    /// weight row.
    #[test]
    fn test_accuracy_matmul() {
        ...
        let pt = encode_int8(&module, &params, &vals);
        let ct = _FHE_LLM_encrypt(&module, &key, &pt, [221u8; 32], [222u8; 32]);

        let weight_rows = vec![vec![2i64, 0, 0, 0], vec![1i64, 1, 0, 0]];

        let results = _FHE_LLM_matmul_single_ct(&module, &ct, &weight_rows);
        assert_eq!(results.len(), 2);

        let pt0 = _FHE_LLM_decrypt(&module, &key, &results[0], &params);
        let dec0 = decode_int8(&module, &params, &pt0, 4);
        let pt1 = _FHE_LLM_decrypt(&module, &key, &results[1], &params);
    ...
    fn test_accuracy_transformer_block_d1() {
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [230u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [231u8; 32], [232u8; 32]);
        ...
        for &input_val in &test_vals {
            ...
            let ct = _FHE_LLM_encrypt(&module, &key, &pt, [233u8; 32], [234u8; 32]);
            let result = _FHE_LLM_transformer_block(&module, &eval_key, &ct, &config, &weights);

            let pt_dec = _FHE_LLM_decrypt(&module, &key, &result, &params);
            let decoded = decode_int8(&module, &params, &pt_dec, 1);
    ...
    fn test_accuracy_error_growth_2_layers() {
        ...
        let input_val: i8 = 5;
        let pt = encode_int8(&module, &params, &[input_val]);
        let ct = _FHE_LLM_encrypt(&module, &key, &pt, [243u8; 32], [244u8; 32]);
    ...
    fn test_accuracy_summary_d4() {
        use crate::arithmetic::{_FHE_LLM_add, _FHE_LLM_matmul_single_ct, _FHE_LLM_mul_const};
        ...
        let key = FHE_LLMKey::generate(&module, &params, [190u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [191u8; 32], [192u8; 32]);

        let vals: Vec<i8> = vec![1, 2, 3, 4];
        let pt = encode_int8(&module, &params, &vals);
        let ct = _FHE_LLM_encrypt(&module, &key, &pt, [183u8; 32], [184u8; 32]);

        // Add
        {
            let vals_b: Vec<i8> = vec![4, 3, 2, 1];
            let pt_b = encode_int8(&module, &params, &vals_b);
            let ct_b = _FHE_LLM_encrypt(&module, &key, &pt_b, [185u8; 32], [186u8; 32]);
            let ct_sum = _FHE_LLM_add(&module, &ct, &ct_b);
            let pt_dec = _FHE_LLM_decrypt(&module, &key, &ct_sum, &params);
            let decoded = decode_int8(&module, &params, &pt_dec, 4);
            let fhe: Vec<f64> = decoded.iter().map(|&x| x as f64).collect();
        ...
        // Mul const (identity)
        {
            ...
            let ct_mul = _FHE_LLM_mul_const(&module, &ct, &identity);
            let pt_dec = _FHE_LLM_decrypt(&module, &key, &ct_mul, &params);
            let decoded = decode_int8(&module, &params, &pt_dec, 4);
            let fhe: Vec<f64> = decoded.iter().map(|&x| x as f64).collect();
        ...
        // Full transformer block
        {
            ...
            let result = _FHE_LLM_transformer_block(&module, &eval_key, &ct, &block_config, &weights);
    ...
    //   cargo +nightly test -p poulpy-FHE_LLM -- --ignored security_sweep
    // ======================================================================
    ...
    fn security_sweep_workload(security: SecurityLevel) -> (f64, f64, f64, bool, f64) {
        ...
        let params = FHE_LLMParams::new(security, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        ...
        let pt_a = encode_int8(&module, &params, &a_vals);
        let pt_b = encode_int8(&module, &params, &b_vals);
        let ct_a = _FHE_LLM_encrypt(&module, &key, &pt_a, [10u8; 32], [11u8; 32]);
        let ct_b = _FHE_LLM_encrypt(&module, &key, &pt_b, [12u8; 32], [13u8; 32]);

        let t0 = std::time::Instant::now();
        let ct_sum = _FHE_LLM_add(&module, &ct_a, &ct_b);
        let add_us = t0.elapsed().as_micros();

        let pt_sum = _FHE_LLM_decrypt(&module, &key, &ct_sum, &params);
        ...
        let ct_m = _FHE_LLM_encrypt(&module, &key, &pt_m, [20u8; 32], [21u8; 32]);
        let constants = vec![2i64, 1, 0, 0];

        let t0 = std::time::Instant::now();
        let ct_mul = _FHE_LLM_mul_const(&module, &ct_m, &constants);
        let mulconst_us = t0.elapsed().as_micros();

        let pt_mul = _FHE_LLM_decrypt(&module, &key, &ct_mul, &params);
        ...
        let ct_for_mul = _FHE_LLM_encrypt(&module, &key, &pt_ct, [30u8; 32], [31u8; 32]);

        let t0 = std::time::Instant::now();
        let ct_prod = _FHE_LLM_ct_mul(&module, &eval_key, &ct_for_mul, &ct_for_mul);
        ...
        let ct_mm = _FHE_LLM_encrypt(&module, &key, &pt_mm, [40u8; 32], [41u8; 32]);
        let weight_rows = vec![vec![2i64, 0, 0, 0], vec![1i64, 1, 0, 0]];

        let t0 = std::time::Instant::now();
        let results_mm = _FHE_LLM_matmul_single_ct(&module, &ct_mm, &weight_rows);
        let matmul_us = t0.elapsed().as_micros();

        let pt_mm0 = _FHE_LLM_decrypt(&module, &key, &results_mm[0], &params);
    ...
    /// Run with: cargo +nightly test -p poulpy-FHE_LLM -- --ignored test_security_sweep_100bit
    #[test]
    #[ignore]
    ...
    /// Run with: cargo +nightly test -p poulpy-FHE_LLM -- --ignored test_security_sweep_128bit
    #[test]
    #[ignore]
    ...
    fn test_qkv_project_vec() {
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [70u8; 32]);
        ...
        let (q_cts, k_cts, v_cts) = crate::attention::_FHE_LLM_qkv_project_vec(&module, &x_cts, &weights, 2, 2);
    ...
    fn test_ffn_standard_vec() {
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [71u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [72u8; 32], [73u8; 32]);
        ...
        let result = _FHE_LLM_ffn_standard_vec(&module, &eval_key, &x_cts, &weights, &ActivationChoice::SquaredReLU);
    ...
    fn test_ffn_swiglu_vec() {
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [74u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [75u8; 32], [76u8; 32]);
        ...
        let result = _FHE_LLM_ffn_swiglu_vec(&module, &eval_key, &x_cts, &weights);
    ...
    fn test_transformer_block_vec_d2() {
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [80u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [81u8; 32], [82u8; 32]);
        ...
        let result = _FHE_LLM_transformer_block_vec(&module, &eval_key, &x_cts, &config, &weights);
    ...
    fn test_forward_pass_vec_2_layers() {
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [85u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [86u8; 32], [87u8; 32]);
        ...
        let result = _FHE_LLM_forward_pass_vec(&module, &eval_key, &x_cts, &config, &layer_weights);
    ...
    fn test_ffn_vec_dispatch() {
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [88u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [89u8; 32], [90u8; 32]);
        ...
        let result_std = _FHE_LLM_ffn_vec(&module, &eval_key, &x_cts, &std_weights, &config_std);
        ...
        let result_swiglu = _FHE_LLM_ffn_vec(&module, &eval_key, &x_cts, &swiglu_weights, &config_swiglu);
        assert_eq!(result_swiglu.len(), 2, "SwiGLU FFN vec dispatch should produce 2 cts");
    }

    /// Tests RMSNorm in vector representation.
    ///
    /// Encrypts a 2-d vector, runs _FHE_LLM_rms_norm_vec, and verifies the
    /// output has the correct dimension count and valid ciphertext layouts.
    #[test]
    fn test_rms_norm_vec() {
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [91u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [92u8; 32], [93u8; 32]);

        // d_model=2 input: [4, 3]
        let x_cts = encrypt_vec(&module, &key, &params, &[4, 3]);

        let config = LayerNormConfig::rms_norm(2);
        let result = crate::layernorm::_FHE_LLM_rms_norm_vec(&module, &eval_key, &x_cts, &config);
    ...
    fn test_rms_norm_vec_numeric_sanity() {
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [141u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [142u8; 32], [143u8; 32]);
    ...
    fn test_multi_head_attention_vec_d4_h2() {
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [94u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [95u8; 32], [96u8; 32]);
        ...
        let result = crate::attention::_FHE_LLM_multi_head_attention_vec(&module, &eval_key, &x_cts, &weights, &attn_config);
    ...
    fn test_multi_head_attention_vec_poly_softmax() {
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [144u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [145u8; 32], [146u8; 32]);
    ...
    fn test_transformer_block_vec_swiglu() {
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [97u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [98u8; 32], [99u8; 32]);
    ...
    /// output rows. Verifies that _FHE_LLM_qkv_project_vec correctly handles
    /// different Q vs KV row counts.
    #[test]
    fn test_gqa_qkv_project_vec() {
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [110u8; 32]);
        ...
        let d_kv = dims.d_kv();
        let (q_cts, k_cts, v_cts) = crate::attention::_FHE_LLM_qkv_project_vec(&module, &x_cts, &weights, dims.d_model, d_kv);
    ...
    /// `_FHE_LLM_multi_head_attention_vec`.
    #[test]
    fn test_gqa_multi_head_attention_vec() {
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [111u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [112u8; 32], [113u8; 32]);
    ...
    /// Exercises `_FHE_LLM_multi_head_attention` which uses a single packed
    /// ciphertext for the input and separate matmul calls for Q/K/V.
    #[test]
    fn test_gqa_multi_head_attention_packed() {
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [114u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [115u8; 32], [116u8; 32]);
        ...
        let pt = encode_int8(&module, &params, &values);
        let ct_x = _FHE_LLM_encrypt(&module, &key, &pt, [117u8; 32], [118u8; 32]);

        let result = crate::attention::_FHE_LLM_multi_head_attention(&module, &eval_key, &ct_x, &weights, &attn_config);
    ...
    fn test_gqa_transformer_block_vec() {
        ...
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [120u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [121u8; 32], [122u8; 32]);
    ...
    fn test_e2e_synthetic_llama_forward_pass() {
        ...
        let result_cts = _FHE_LLM_transformer_block_vec(&module, &eval_key, &ct_x, &block_config, &vec_weights);
    ...
    fn run_e2e_real_tinyllama_fhe_inference_d64(security: SecurityLevel) {
        ...
        let result_cts = _FHE_LLM_transformer_block_vec(&module, &eval_key, &ct_x, &block_config, &trunc_weights);
        ...
        let (fhe_q, fhe_k, _fhe_v) =
            crate::attention::_FHE_LLM_qkv_project_vec(&module, &ct_x, &trunc_weights.attention, trunc_d_model, trunc_d_model);
    ...
    /// Tests that RoPE is correctly wired into `_FHE_LLM_multi_head_attention_vec`.
    ///
    ...
    fn test_multi_head_attention_vec_with_rope() {
        ...
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [90u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [91u8; 32], [92u8; 32]);
        ...
        let result_no_rope =
            crate::attention::_FHE_LLM_multi_head_attention_vec(&module, &eval_key, &x_cts, &weights, &config_no_rope);
        let out_no_rope = decrypt_vec(&module, &key, &params, &result_no_rope);

        // Run attention with RoPE
        let result_with_rope =
            crate::attention::_FHE_LLM_multi_head_attention_vec(&module, &eval_key, &x_cts, &weights, &config_with_rope);
    ...
    fn test_transformer_block_vec_with_rope() {
        ...
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [93u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [94u8; 32], [95u8; 32]);
    ...
    /// Tests `_FHE_LLM_forward_pass_vec_full` with final RMSNorm and no bootstrapping.
    ///
    ...
    fn test_forward_pass_vec_full_with_final_norm() {
        ...
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [96u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [97u8; 32], [98u8; 32]);
        ...
        let (result_full, tracker) =
            _FHE_LLM_forward_pass_vec_full(&module, &eval_key, &x_cts, &fp_config, &[weights.clone()], None);
        ...
        let result_plain = _FHE_LLM_forward_pass_vec(&module, &eval_key, &x_cts, &block_config, &[weights]);
    ...
    /// Tests `_FHE_LLM_forward_pass_vec_full` with 2 layers and RoPE.
    ///
    ...
    fn test_forward_pass_vec_full_2_layers_with_rope() {
        ...
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [99u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [100u8; 32], [101u8; 32]);
        ...
        let (result, tracker) = _FHE_LLM_forward_pass_vec_full(&module, &eval_key, &x_cts, &fp_config, &layer_weights, None);
    ...
    fn test_plaintext_transformer_block_d4_identity() {
        ...
        let config = TransformerBlockConfig {
            attention: AttentionConfig {
                dims: dims.clone(),
                params: FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8),
                softmax_approx: SoftmaxStrategy::ReluSquared,
                causal: true,
                rope: None,
            },
    ...
    fn test_fhe_vs_plaintext_comparison_d4() {
        ...
        // --- FHE path ---
        let ct_x = encrypt_vec(&module, &key, &params, &input_i8);
        let fhe_result = _FHE_LLM_transformer_block_vec(&module, &eval_key, &ct_x, &config, &weights);
    ...
    fn test_align_layout_preserves_scalar_value() {
        ...
        let aligned = crate::arithmetic::_FHE_LLM_align_layout(&module, &ct, &target);

        let pt_dec = _FHE_LLM_decrypt(&module, &key, &aligned, &params);
        let decoded = decode_int8(&module, &params, &pt_dec, 1);
        eprintln!(
    ...
    /// grows with depth. This validates that FHE_LLM's noise budget is
    ...
    fn test_noise_diag_single_mul_const() {
        ...
        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [170u8; 32]);
        ...
        for (val, weight) in &test_cases {
            ...
            let pt = encode_int8(&module, &params, &vals);
            let ct = _FHE_LLM_encrypt(&module, &key, &pt, [171u8; 32], [172u8; 32]);

            let ct_mul = _FHE_LLM_mul_const(&module, &ct, &[*weight]);
    ...
    fn test_noise_diag_dot_product_scaling() {
        use crate::arithmetic::_FHE_LLM_dot_product;

        let params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Int8);
        let module: Module<BE> = Module::new(params.n());
        let key = FHE_LLMKey::generate(&module, &params, [180u8; 32]);
        ...
        for d in [1, 2, 4, 8, 16, 32, 64] {
            ...
            let ct_dot = _FHE_LLM_dot_product(&module, &cts, &weights);
            let pt_dec = _FHE_LLM_decrypt(&module, &key, &ct_dot, &params);
            let decoded = decode_int8(&module, &params, &pt_dec, 1);
    ...
    /// Test the effect of res_offset on noise in _FHE_LLM_mul_const.
    /// Currently FHE_LLM uses res_offset = ct.base2k() = in_base2k = 13.
    ...
    fn test_noise_diag_res_offset_comparison() {
        ...
        let vals: Vec<i8> = vec![val];
        let pt = encode_int8(&module, &params, &vals);
        let ct = _FHE_LLM_encrypt(&module, &key, &pt, [201u8; 32], [202u8; 32]);
        ...
        // Current FHE_LLM choice: res_offset = in_base2k = 13
        let res_offset_current = params.in_base2k();
        {
            ...
            let pt_dec = _FHE_LLM_decrypt(&module, &key, &res, &params);
            let decoded = decode_int8(&module, &params, &pt_dec, 1);
            eprintln!(
                "{:>12} {:>10} {:>10} {:>20}",
                res_offset_current,
    ...
    fn test_noise_diag_ct_ct_multiply() {
        ...
        let key = FHE_LLMKey::generate(&module, &params, [210u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [211u8; 32], [212u8; 32]);
        ...
        for &(a, b) in &test_cases {
            ...
            let ct_a = _FHE_LLM_encrypt(&module, &key, &pt_a, [213u8; 32], [214u8; 32]);
            ...
            let ct_b = _FHE_LLM_encrypt(&module, &key, &pt_b, [215u8; 32], [216u8; 32]);

            let ct_mul = _FHE_LLM_ct_mul(&module, &eval_key, &ct_a, &ct_b);
    ...
    fn test_noise_diag_chain_linear_nonlinear() {
        ...
        eprintln!("\n--- Test 1: encode_vec_i64 at TorusPrecision({scale}) ---");
        {
            ...
            let mut pt = GLWEPlaintext::<Vec<u8>>::alloc_from_infos(&pt_layout);
            pt.encode_vec_i64(&data, TorusPrecision(scale as u32));
            let ct = _FHE_LLM_encrypt(&module, &key, &pt, [223u8; 32], [224u8; 32]);

            // Step 1: mul_const
            let ct_mul = _FHE_LLM_mul_const(&module, &ct, &[weight]);
            // Decrypt and decode at scale
            let pt_dec1 = _FHE_LLM_decrypt(&module, &key, &ct_mul, &params);
            ...
            let ct_sq = _FHE_LLM_ct_mul(&module, &eval_key, &ct_mul, &ct_mul);
        ...
        eprintln!("\n--- Test 2: encode_int8 (in_base2k={}) ---", params.in_base2k());
        {
            ...
            let ct = _FHE_LLM_encrypt(&module, &key, &pt, [225u8; 32], [226u8; 32]);
        ...
        eprintln!("\n--- Test 3: Step-by-step transformer block diagnostic ---");
        {
            ...
            // Step 1: RMSNorm
            let normed = _FHE_LLM_rms_norm_vec(&module, &eval_key, &ct_x, &config.pre_attn_norm);
            eprintln!("[diag] After pre-attn RMSNorm:");
            for (i, ct) in normed.iter().enumerate() {
                let pt_out = _FHE_LLM_decrypt(&module, &key, ct, &params);
                // Try multiple decode precisions
        ...
        eprintln!("\n--- Test 3b: encode_int8 + manual upscale before ct×ct ---");
        {
            ...
            let ct_a = _FHE_LLM_encrypt(&module, &key, &pt_a, [230u8; 32], [231u8; 32]);
            let ct_sq_a = _FHE_LLM_ct_mul(&module, &eval_key, &ct_a, &ct_a);
            ...
            let ct_b = _FHE_LLM_encrypt(&module, &key, &pt_b, [232u8; 32], [233u8; 32]);
            let ct_sq_b = _FHE_LLM_ct_mul(&module, &eval_key, &ct_b, &ct_b);
