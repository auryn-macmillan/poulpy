// Test at d=128, 30 layers with homomorphic LM head
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    eprintln!("=== SmolLM2 Validation (d=128, layers={n_layers}, HOMOMORPHIC LM HEAD) ===");
    ...
    eprintln!("[fhe] This will take ~30-40 minutes for 30 layers at d=128");
    eprintln!();
    ...
    if fhe_result.token_id == 33 {
        ...
        eprintln!("\n❌ FAIL: Expected token 33 (\"1\"), got {}", fhe_result.token_id);
        eprintln!("   Top logits: {:?}", &fhe_result.top_logits[..5]);
    }
