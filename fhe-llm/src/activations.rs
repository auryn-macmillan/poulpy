...
//! are co-designed with the FHE_LLM scheme to minimise multiplicative depth
...
use crate::arithmetic::_FHE_LLM_align_layout;
    ...
    Scratch<BE>: ScratchTakeCore<BE> + ScratchAvailable,
{
    ...
    let ct_for_cube = if ct.base2k() == ct_sq.base2k() && ct.k() == ct_sq.k() {
        ...
        _FHE_LLM_align_layout(module, ct, &ct_sq.glwe_layout())
    };
    let ct_cube = _FHE_LLM_ct_mul(module, eval_key, &ct_sq, &ct_for_cube);
    ...
    let ct_sq_for_fourth = if ct_sq.base2k() == ct_cube.base2k() && ct_sq.k() == ct_cube.k() {
        ...
        _FHE_LLM_align_layout(module, &ct_sq, &ct_cube.glwe_layout())
    };
    let ct_fourth = _FHE_LLM_ct_mul(module, eval_key, &ct_sq_for_fourth, &ct_sq_for_fourth);
...
/// `_FHE_LLM_align_layout`. This ensures:
///
...
pub fn _FHE_LLM_ct_mul<BE: Backend>(
    module: &Module<BE>,
    eval_key: &FHE_LLMEvalKey<BE>,
    a: &GLWE<Vec<u8>>,
    b: &GLWE<Vec<u8>>,
...
where
    ...
    Scratch<BE>: ScratchTakeCore<BE> + ScratchAvailable,
{
    _FHE_LLM_ct_mul_with_res_offset(module, eval_key, a, b, eval_key.res_offset)
}

pub fn _FHE_LLM_ct_mul_with_res_offset<BE: Backend>(
    module: &Module<BE>,
    eval_key: &FHE_LLMEvalKey<BE>,
    a: &GLWE<Vec<u8>>,
    b: &GLWE<Vec<u8>>,
    ...
    Scratch<BE>: ScratchTakeCore<BE> + ScratchAvailable,
{
    ...
    let a_proj: std::borrow::Cow<'_, GLWE<Vec<u8>>> = if a.base2k() != in_base2k_b2k {
        ...
        std::borrow::Cow::Owned(crate::arithmetic::_FHE_LLM_align_layout(module, a, &target))
    } else {
    ...
    let b_proj: std::borrow::Cow<'_, GLWE<Vec<u8>>> = if b.base2k() != in_base2k_b2k {
        ...
        std::borrow::Cow::Owned(crate::arithmetic::_FHE_LLM_align_layout(module, b, &target))
    } else {
