...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let baseline = pipeline.compare_prompt_fhe_first_layer_stages_with_pre_ffn_config(
        &prompt,
        attn_decode,
        residual_decode,
        LayerNormConfig::rms_norm_midrange(d),
    )?;
    let range32_result =
        pipeline.compare_prompt_fhe_first_layer_stages_with_pre_ffn_config(&prompt, attn_decode, residual_decode, range32)?;
