...
fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    println!("=== Split-Point FHE Inference Debug ===");
    ...
    println!("Ciphertext n={}, base2k={}, k={}", 
        hidden_encrypted[0].n(), 
        hidden_encrypted[0].base2k(), 
        hidden_encrypted[0].k());

    let hidden_cleartext = pipeline.decrypt_vec(&hidden_encrypted)?;
    
    println!("Decrypted hidden state first 16: {:?}", &hidden_cleartext[..16.min(hidden_cleartext.len())]);
    println!("Decrypted hidden state stats: min={}, max={}, sum={}", 
        hidden_cleartext.iter().min().unwrap(),
        hidden_cleartext.iter().max().unwrap(),
        hidden_cleartext.iter().sum::<i8>());
