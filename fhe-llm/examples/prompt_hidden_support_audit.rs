...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let dims = args.next().map(|s| s.parse::<usize>()).transpose()?.unwrap_or(4);
    ...
    let diag = pipeline.diagnose_prompt_first_layer_polynomial_support_with_pre_ffn_config(
        &prompt,
        attn_decode,
        residual_decode,
        range32,
    ...
    println!(
        "config: attn_decode={} residual_decode={} dims={}",
        attn_decode, residual_decode, dims
    );
    for row in diag.rows {
        println!("{}: {:.3}", row.stage, row.value);
    }
