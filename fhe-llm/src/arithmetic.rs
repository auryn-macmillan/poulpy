//! Homomorphic arithmetic operations for FHE_LLM.
//!
...
/// Adds two FHE_LLM ciphertexts element-wise.
///
...
pub fn _FHE_LLM_add<BE: Backend>(module: &Module<BE>, a: &GLWE<Vec<u8>>, b: &GLWE<Vec<u8>>) -> GLWE<Vec<u8>>
where
    Module<BE>: GLWEAdd,
{
    assert_eq!(a.n(), b.n());
...
/// Subtracts two FHE_LLM ciphertexts element-wise.
///
/// Computes `res = a - b`.
///
/// # Panics
///
/// Panics if the ciphertexts have incompatible parameters.
pub fn _FHE_LLM_sub<BE: Backend>(module: &Module<BE>, a: &GLWE<Vec<u8>>, b: &GLWE<Vec<u8>>) -> GLWE<Vec<u8>>
where
    Module<BE>: GLWESub,
{
    assert_eq!(a.n(), b.n());
...
/// Multiplies a FHE_LLM ciphertext by a plaintext constant polynomial.
///
...
pub fn _FHE_LLM_mul_const<BE: Backend>(module: &Module<BE>, ct: &GLWE<Vec<u8>>, constants: &[i64]) -> GLWE<Vec<u8>>
where
...
/// Multiplies a FHE_LLM ciphertext by a plaintext constant polynomial and
/// compensates for an extra fixed-point scale factor carried by the ciphertext.
pub fn _FHE_LLM_mul_const_scaled<BE: Backend>(
    module: &Module<BE>,
    ct: &GLWE<Vec<u8>>,
    constants: &[i64],
    ...
    Scratch<BE>: ScratchAvailable,
{
    ...
    assert!(
        ...
        "_FHE_LLM_mul_const_scaled: scale_shift_bits ({scale_shift_bits}) must be <= base2k ({base2k})"
    );
...
/// Multiplies a FHE_LLM ciphertext by a plaintext constant with a torus bit-shift.
///
...
/// * `shift_bits` - Number of torus bits to right-shift (0 = identity, same as `_FHE_LLM_mul_const`).
///
/// # Panics
///
/// Panics if `shift_bits >= ct.base2k()` (would shift away all signal).
pub fn _FHE_LLM_mul_const_with_shift<BE: Backend>(
    module: &Module<BE>,
    ct: &GLWE<Vec<u8>>,
    constants: &[i64],
    shift_bits: usize,
    ...
    Scratch<BE>: ScratchAvailable,
{
    ...
    assert!(
        shift_bits < base2k,
        "_FHE_LLM_mul_const_with_shift: shift_bits ({shift_bits}) must be < base2k ({base2k})"
    );
...
/// Multiplies a FHE_LLM ciphertext by a plaintext constant and left-shifts the
/// torus representation by `shift_bits`.
pub fn _FHE_LLM_mul_const_with_left_shift<BE: Backend>(
    module: &Module<BE>,
    ct: &GLWE<Vec<u8>>,
    constants: &[i64],
    shift_bits: usize,
...
/// into the requested layout. This is an alignment helper for FHE_LLM's tested
...
pub fn _FHE_LLM_align_layout<BE: Backend>(module: &Module<BE>, ct: &GLWE<Vec<u8>>, target: &GLWELayout) -> GLWE<Vec<u8>>
where
    Module<BE>: GLWEMulConst<BE>,
    ScratchOwned<BE>: ScratchOwnedAlloc<BE> + ScratchOwnedBorrow<BE>,
    Scratch<BE>: ScratchAvailable,
...
#[deprecated(note = "use _FHE_LLM_align_layout instead")]
pub fn _FHE_LLM_project_layout<BE: Backend>(module: &Module<BE>, ct: &GLWE<Vec<u8>>, target: &GLWELayout) -> GLWE<Vec<u8>>
where
    Module<BE>: GLWEMulConst<BE>,
    ScratchOwned<BE>: ScratchOwnedAlloc<BE> + ScratchOwnedBorrow<BE>,
    Scratch<BE>: ScratchAvailable,
{
    _FHE_LLM_align_layout(module, ct, target)
}

/// Rescales a FHE_LLM ciphertext, reducing the scale factor by 2^base2k.
///
...
pub fn _FHE_LLM_rescale<BE: Backend>(_module: &Module<BE>, ct: &GLWE<Vec<u8>>, params: &FHE_LLMParams) -> GLWE<Vec<u8>>
where
    Module<BE>: ModuleN,
{
...
pub fn _FHE_LLM_dot_product<BE: Backend>(module: &Module<BE>, cts: &[GLWE<Vec<u8>>], weights: &[Vec<i64>]) -> GLWE<Vec<u8>>
where
    ...
    Scratch<BE>: ScratchAvailable,
{
    ...
    let mut acc = _FHE_LLM_mul_const(module, &cts[0], &weights[0]);
...
pub fn _FHE_LLM_dot_product_scaled<BE: Backend>(
    module: &Module<BE>,
    cts: &[GLWE<Vec<u8>>],
    weights: &[Vec<i64>],
    scale_shift_bits: usize,
    ...
    Scratch<BE>: ScratchAvailable,
{
    ...
    let mut acc = _FHE_LLM_mul_const_scaled(module, &cts[0], &weights[0], scale_shift_bits);
...
///
/// Panics if scratch space is insufficient.
pub fn _FHE_LLM_slot_sum<BE: Backend>(
    module: &Module<BE>,
...
/// Call [`FHE_LLMEvalKey::add_rotation_keys`] with the needed positions before
/// using this function.
pub fn _FHE_LLM_rotate_slots<BE: Backend>(
    module: &Module<BE>,
    eval_key: &FHE_LLMEvalKey<BE>,
    ct: &GLWE<Vec<u8>>,
    k: i64,
    ...
    Scratch<BE>: ScratchTakeCore<BE> + ScratchAvailable,
{
    ...
    let auto_key = eval_key.auto_keys.get(&gal_el).unwrap_or_else(|| {
        panic!(
            "_FHE_LLM_rotate_slots: no automorphism key for rotation k={k} \
                 (Galois element {gal_el}). Call FHE_LLMEvalKey::add_rotation_keys \
...
/// `_FHE_LLM_mul_const` (which multiplies all slots by a constant polynomial).
///
/// For multi-ciphertext input (values spread across multiple cts), this
/// computes `_FHE_LLM_dot_product` per output row.
///
/// # Arguments
///
/// * `module` - Backend module.
...
pub fn _FHE_LLM_matmul_ct_pt<BE: Backend>(
    module: &Module<BE>,
    x_cts: &[GLWE<Vec<u8>>],
    weights: &[Vec<Vec<i64>>],
) -> Vec<GLWE<Vec<u8>>>
    ...
    Scratch<BE>: ScratchAvailable,
{
    ...
    for row in weights.iter() {
        ...
        // _FHE_LLM_dot_product computes Σ ct_i * w_i
        let out = _FHE_LLM_dot_product(module, x_cts, row);
        outputs.push(out);
    }
...
/// must sum slots (via `_FHE_LLM_slot_sum`) to obtain the scalar dot product.
pub fn _FHE_LLM_matmul_single_ct<BE: Backend>(
    module: &Module<BE>,
    ct: &GLWE<Vec<u8>>,
    weight_rows: &[Vec<i64>],
) -> Vec<GLWE<Vec<u8>>>
    ...
    Scratch<BE>: ScratchAvailable,
{
    ...
    for row in weight_rows.iter() {
        let out = _FHE_LLM_mul_const(module, ct, row);
        outputs.push(out);
    }
...
mod tests {
    ...
    fn test_sub() {
        ...
        let ct_diff = _FHE_LLM_sub(&module, &ct_a, &ct_b);
        let pt_diff = _FHE_LLM_decrypt(&module, &key, &ct_diff, &params);
    ...
    fn test_matmul_single_ct() {
        // Test _FHE_LLM_matmul_single_ct: multiply a single encrypted vector
        ...
        let ct = _FHE_LLM_encrypt(&module, &key, &pt, [10u8; 32], [11u8; 32]);
    ...
    fn test_matmul_ct_pt_multi() {
        // Test _FHE_LLM_matmul_ct_pt with multiple input ciphertexts.
        ...
        let results = _FHE_LLM_matmul_ct_pt(&module, &x_cts, &weights);
    ...
    fn test_rotate_slots_identity() {
        ...
        let eval_key = crate::encrypt::FHE_LLMEvalKey::generate(&module, &key, &params, [4u8; 32], [5u8; 32]);

        let ct_rot = _FHE_LLM_rotate_slots(&module, &eval_key, &ct, 0);
        let pt_dec = _FHE_LLM_decrypt(&module, &key, &ct_rot, &params);
    ...
    fn test_rotate_slots_by_one() {
        ...
        // For coefficient-domain encoding (as used in FHE_LLM), this applies
        ...
        let mut eval_key = crate::encrypt::FHE_LLMEvalKey::generate(&module, &key, &params, [4u8; 32], [5u8; 32]);
        eval_key.add_rotation_keys(&module, &key, &[1], [6u8; 32], [7u8; 32]);

        let ct_rot = _FHE_LLM_rotate_slots(&module, &eval_key, &ct, 1);
        let pt_dec = _FHE_LLM_decrypt(&module, &key, &ct_rot, &params);
