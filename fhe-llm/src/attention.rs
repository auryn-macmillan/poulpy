...
//! decoder layer, with all operations expressed in terms of FHE_LLM's
...
//!
//! - **QKV projection**: `_FHE_LLM_matmul_single_ct` with plaintext weight rows
//! - **Score computation**: `_FHE_LLM_ct_mul` (Q·K element product) + `_FHE_LLM_slot_sum` (reduction)
//! - **Softmax**: `apply_poly_activation` with the selected softmax polynomial
//! - **Context**: `_FHE_LLM_ct_mul` (attn_weights · V) per head
//! - **Output projection**: `_FHE_LLM_matmul_single_ct` with plaintext W_O

use crate::activations::{apply_poly_activation, _FHE_LLM_ct_mul, squared_relu_approx, PolyApprox};
use crate::arithmetic::{_FHE_LLM_add, _FHE_LLM_matmul_single_ct, _FHE_LLM_mul_const, _FHE_LLM_slot_sum, _FHE_LLM_sub};
...
pub struct AttentionConfig {
    ...
    /// and before score computation in `_FHE_LLM_multi_head_attention_vec`.
...
pub fn _FHE_LLM_apply_rope_vec<BE: Backend>(module: &Module<BE>, q_or_k: &[GLWE<Vec<u8>>], rope: &RoPEConfig) -> Vec<GLWE<Vec<u8>>>
where
    ...
    Scratch<BE>: ScratchAvailable,
{
    ...
    assert!(
        d_head % 2 == 0,
        "_FHE_LLM_apply_rope_vec: input length ({d_head}) must be even"
    );
    assert_eq!(
        d_head,
        rope.cos_table.len() * 2,
        "_FHE_LLM_apply_rope_vec: input length ({d_head}) != 2 * cos_table.len() ({})",
        rope.cos_table.len() * 2
    );
    ...
    let out: Vec<GLWE<Vec<u8>>> = {
        ...
        (0..n_pairs)
            ...
            .flat_map(|i| {
                ...
                // q[2i] * cos[i]
                let even_cos = _FHE_LLM_mul_const(module, &q_or_k[2 * i], &[cos_i]);
                // q[2i+1] * sin[i]
                let odd_sin = _FHE_LLM_mul_const(module, &q_or_k[2 * i + 1], &[sin_i]);
                // out[2i] = q[2i] * cos[i] - q[2i+1] * sin[i]
                let out_even = _FHE_LLM_sub(module, &even_cos, &odd_sin);

                // q[2i] * sin[i]
                let even_sin = _FHE_LLM_mul_const(module, &q_or_k[2 * i], &[sin_i]);
                // q[2i+1] * cos[i]
                let odd_cos = _FHE_LLM_mul_const(module, &q_or_k[2 * i + 1], &[cos_i]);
                // out[2i+1] = q[2i] * sin[i] + q[2i+1] * cos[i]
                let out_odd = _FHE_LLM_add(module, &even_sin, &odd_cos);
...
///
/// Each projection is computed as `_FHE_LLM_mul_const(ct_x, w_row)` where
...
pub fn _FHE_LLM_qkv_project_single<BE: Backend>(
    module: &Module<BE>,
    ct_x: &GLWE<Vec<u8>>,
    w_q_row: &[i64],
    w_k_row: &[i64],
...
where
    ...
    Scratch<BE>: ScratchAvailable,
{
    let q = _FHE_LLM_mul_const(module, ct_x, w_q_row);
    let k = _FHE_LLM_mul_const(module, ct_x, w_k_row);
    let v = _FHE_LLM_mul_const(module, ct_x, w_v_row);
    (q, k, v)
}
...
/// `_FHE_LLM_matmul_single_ct` with the weight rows. Each output ciphertext
...
/// single-ct variant in [`_FHE_LLM_multi_head_attention`] which handles
/// separate Q/KV row counts directly.
#[allow(dead_code)]
pub fn _FHE_LLM_qkv_project<BE: Backend>(
    module: &Module<BE>,
    ct_x: &GLWE<Vec<u8>>,
    ...
    Scratch<BE>: ScratchAvailable,
{
    ...
    let q_cts = _FHE_LLM_matmul_single_ct(module, ct_x, &w_q_rows);
    let k_cts = _FHE_LLM_matmul_single_ct(module, ct_x, &w_k_rows);
    let v_cts = _FHE_LLM_matmul_single_ct(module, ct_x, &w_v_rows);
...
pub fn _FHE_LLM_attention_score<BE: Backend>(
    module: &Module<BE>,
    ...
    Scratch<BE>: ScratchTakeCore<BE> + ScratchAvailable,
{
    ...
    let qk = _FHE_LLM_ct_mul(module, eval_key, ct_q, ct_k);

    // Step 2: Sum all slots to get the dot product
    _FHE_LLM_slot_sum(module, eval_key, &qk, skip_trace)
}
...
pub fn _FHE_LLM_apply_softmax<BE: Backend>(
    module: &Module<BE>,
    eval_key: &FHE_LLMEvalKey<BE>,
    scores: &[GLWE<Vec<u8>>],
...
pub fn _FHE_LLM_attention_context<BE: Backend>(
    module: &Module<BE>,
    eval_key: &FHE_LLMEvalKey<BE>,
    ...
    Scratch<BE>: ScratchTakeCore<BE> + ScratchAvailable,
{
    ...
    let mut acc = _FHE_LLM_ct_mul(module, eval_key, &attn_weights[0], &v_cts[0]);

    for i in 1..attn_weights.len() {
        let term = _FHE_LLM_ct_mul(module, eval_key, &attn_weights[i], &v_cts[i]);
        acc = _FHE_LLM_add(module, &acc, &term);
    }
...
pub fn _FHE_LLM_output_project<BE: Backend>(
    module: &Module<BE>,
...
where
    ...
    Scratch<BE>: ScratchAvailable,
{
    _FHE_LLM_matmul_single_ct(module, ct_context, w_o_rows)
}
...
/// - Accumulating with _FHE_LLM_add to compute the dot product
///
...
/// inconsistent.
pub fn _FHE_LLM_multi_head_attention<BE: Backend>(
    module: &Module<BE>,
    eval_key: &FHE_LLMEvalKey<BE>,
    ct_x: &GLWE<Vec<u8>>,
    weights: &AttentionWeights,
    ...
    Scratch<BE>: ScratchTakeCore<BE> + ScratchAvailable,
{
    ...
    assert_eq!(
        d_model,
        n_heads * d_head,
        "_FHE_LLM_multi_head_attention: d_model ({d_model}) != n_heads ({n_heads}) * d_head ({d_head})"
    );
    assert!(
        weights.w_q.len() >= d_model,
        "_FHE_LLM_multi_head_attention: w_q has {} rows, need {d_model}",
        weights.w_q.len()
    );
    assert!(
        weights.w_k.len() >= d_kv,
        "_FHE_LLM_multi_head_attention: w_k has {} rows, need {d_kv}",
        weights.w_k.len()
    );
    assert!(
        weights.w_v.len() >= d_kv,
        "_FHE_LLM_multi_head_attention: w_v has {} rows, need {d_kv}",
        weights.w_v.len()
    );
    assert!(
        weights.w_o.len() >= d_model,
        "_FHE_LLM_multi_head_attention: w_o has {} rows, need {d_model}",
        weights.w_o.len()
    );
    ...
    // -----------------------------------------------------------------------
    let q_all = _FHE_LLM_matmul_single_ct(module, ct_x, &weights.w_q[..d_model].to_vec());
    let k_all = _FHE_LLM_matmul_single_ct(module, ct_x, &weights.w_k[..d_kv].to_vec());
    let v_all = _FHE_LLM_matmul_single_ct(module, ct_x, &weights.w_v[..d_kv].to_vec());
    ...
    for h in 0..n_heads {
        ...
        let head_context = _FHE_LLM_single_head_attention(module, eval_key, q_h, k_h, v_h, &config.softmax_approx);
    ...
    //
    // This is a ct-pt matrix-vector product using _FHE_LLM_dot_product.
    // -----------------------------------------------------------------------
    ...
    for row in weights.w_o.iter().take(d_model) {
        ...
        let dot = crate::arithmetic::_FHE_LLM_dot_product(module, &all_head_contexts, &w_vecs);
        outputs.push(dot);
    }
...
pub fn _FHE_LLM_single_head_attention<BE: Backend>(
    _module: &Module<BE>,
    _eval_key: &FHE_LLMEvalKey<BE>,
    q_h: &[GLWE<Vec<u8>>],
    k_h: &[GLWE<Vec<u8>>],
    v_h: &[GLWE<Vec<u8>>],
...
/// is computed as a [`_FHE_LLM_dot_product`] per output dimension.
///
...
pub fn _FHE_LLM_qkv_project_vec<BE: Backend>(
    module: &Module<BE>,
    x_cts: &[GLWE<Vec<u8>>],
    weights: &AttentionWeights,
    n_q_rows: usize,
...
where
    ...
    Scratch<BE>: ScratchAvailable,
{
    _FHE_LLM_qkv_project_vec_scaled(module, x_cts, weights, n_q_rows, n_kv_rows, 0)
}

pub fn _FHE_LLM_qkv_project_vec_scaled<BE: Backend>(
    module: &Module<BE>,
    x_cts: &[GLWE<Vec<u8>>],
    weights: &AttentionWeights,
    n_q_rows: usize,
...
///
/// This is the vector-representation variant of [`_FHE_LLM_multi_head_attention`].
...
pub fn _FHE_LLM_multi_head_attention_vec<BE: Backend>(
    module: &Module<BE>,
    eval_key: &FHE_LLMEvalKey<BE>,
    x_cts: &[GLWE<Vec<u8>>],
    weights: &AttentionWeights,
    ...
    Scratch<BE>: ScratchTakeCore<BE> + ScratchAvailable,
{
    ...
    assert_eq!(
        d_model,
        n_heads * d_head,
        "_FHE_LLM_multi_head_attention_vec: d_model ({d_model}) != n_heads ({n_heads}) * d_head ({d_head})"
    );
    assert_eq!(
        x_cts.len(),
        d_model,
        "_FHE_LLM_multi_head_attention_vec: input has {} cts, expected {d_model}",
        x_cts.len()
    );
    ...
    let (q_all, k_all, v_all) = _FHE_LLM_qkv_project_vec_scaled(
        module,
        x_cts,
        weights,
        d_model,
    ...
    let (q_all, k_all) = if let Some(rope) = &config.rope {
        ...
        let q_rope: Vec<GLWE<Vec<u8>>> = (0..n_heads)
            ...
            .flat_map(|h| {
                let q_start = h * d_head;
                let q_end = q_start + d_head;
                _FHE_LLM_apply_rope_vec(module, &q_all[q_start..q_end], rope)
            })
            .collect();
        ...
        let k_rope: Vec<GLWE<Vec<u8>>> = (0..n_kv_heads)
            ...
            .flat_map(|kv_h| {
                let kv_start = kv_h * d_head;
                let kv_end = kv_start + d_head;
                _FHE_LLM_apply_rope_vec(module, &k_all[kv_start..kv_end], rope)
            })
            .collect();
    ...
    let all_head_contexts: Vec<GLWE<Vec<u8>>> = if n_heads > 1 {
        ...
        let head_results: Vec<Vec<GLWE<Vec<u8>>>> = (0..n_heads)
            ...
            .map(|h| {
                ...
                _FHE_LLM_single_head_attention(module, eval_key, q_h, k_h, v_h, &config.softmax_approx)
            })
            .collect();
    ...
    let outputs: Vec<GLWE<Vec<u8>>> = {
        ...
        weights
            ...
            .map(|row| {
                let w_vecs: Vec<Vec<i64>> = row.iter().map(|&w| vec![w]).collect();
                crate::arithmetic::_FHE_LLM_dot_product(module, &all_head_contexts, &w_vecs)
            })
            .collect()
    };
...
mod tests {
    ...
    fn test_qkv_project_single() {
        ...
        let (ct_q, ct_k, ct_v) = _FHE_LLM_qkv_project_single(&module, &ct, &[2i64], &[3i64], &[1i64]);

        let pt_q = _FHE_LLM_decrypt(&module, &key, &ct_q, &params);
        let pt_k = _FHE_LLM_decrypt(&module, &key, &ct_k, &params);
        let pt_v = _FHE_LLM_decrypt(&module, &key, &ct_v, &params);
    ...
    fn test_attention_score_basic() {
        ...
        let ct_q = _FHE_LLM_encrypt(&module, &key, &pt_q, [10u8; 32], [11u8; 32]);
        let ct_k = _FHE_LLM_encrypt(&module, &key, &pt_k, [12u8; 32], [13u8; 32]);

        // Compute attention score (ct*ct mul + slot sum)
        let score = _FHE_LLM_attention_score(&module, &eval_key, &ct_q, &ct_k, 0);
    ...
    fn test_apply_softmax_linear() {
        ...
        let results = _FHE_LLM_apply_softmax(&module, &eval_key, &[ct], &SoftmaxStrategy::Linear);
        assert_eq!(results.len(), 1);

        // Linear softmax should preserve the values
        let pt_dec = _FHE_LLM_decrypt(&module, &key, &results[0], &params);
    ...
    fn test_rope_apply_identity() {
        ...
        for (idx, &v) in values.iter().enumerate() {
            ...
            let ct = _FHE_LLM_encrypt(&module, &key, &pt, seed_a, seed_e);
            cts.push(ct);
        }
        ...
        // Apply RoPE
        let rotated = _FHE_LLM_apply_rope_vec(&module, &cts, &rope);
        ...
        for (idx, &v) in values.iter().enumerate() {
            let pt_dec = _FHE_LLM_decrypt(&module, &key, &rotated[idx], &params);
