...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let sweep = pipeline.sweep_prompt_fhe_decode_precision(&prompt, &precisions)?;
    ...
    for row in sweep.rows {
        println!(
            "precision={}: token={} {:?} | L-inf={:.3} L2={:.3} MAE={:.3} | plaintext_rank={} pred_logit={:.4} plaintext_logit={:.4}",
            row.precision,
            row.token_id,
            row.token_text,
            row.linf,
