//! MAC-based user-side verification for FHE_LLM.
//!
...
//!
//! Without MAC verification (FHE_LLM v1):
...
use crate::arithmetic::{_FHE_LLM_add, _FHE_LLM_mul_const};
use crate::encrypt::FHE_LLMKey;
...
pub fn _FHE_LLM_mac_tag<BE: Backend>(module: &Module<BE>, mac_key: &MacKey, ct: &GLWE<Vec<u8>>) -> TaggedCiphertext<Vec<u8>>
where
    ...
    Scratch<BE>: ScratchAvailable,
{
    ...
    let tag = _FHE_LLM_mul_const(module, ct, &[mac_key.alpha]);
...
///
/// Convenience wrapper that applies [`_FHE_LLM_mac_tag`] to each ciphertext
/// in the vector.
pub fn _FHE_LLM_mac_tag_vec<BE: Backend>(
    module: &Module<BE>,
    mac_key: &MacKey,
    cts: &[GLWE<Vec<u8>>],
...
where
    ...
    Scratch<BE>: ScratchAvailable,
{
    cts.iter().map(|ct| _FHE_LLM_mac_tag(module, mac_key, ct)).collect()
}
...
{
    TaggedCiphertext {
        ct: _FHE_LLM_add(module, &a.ct, &b.ct),
        tag: _FHE_LLM_add(module, &a.tag, &b.tag),
    }
}
...
{
    TaggedCiphertext {
        ct: _FHE_LLM_mul_const(module, &tagged.ct, constants),
        tag: _FHE_LLM_mul_const(module, &tagged.tag, constants),
    }
}
    ...
    Scratch<BE>: ScratchAvailable,
{
    ...
    let mut ct_acc = _FHE_LLM_mul_const(module, cts[0], &weights[0]);
    for i in 1..cts.len() {
        let term = _FHE_LLM_mul_const(module, cts[i], &weights[i]);
        ct_acc = _FHE_LLM_add(module, &ct_acc, &term);
    }

    // Compute tag dot product (same weights)
    let mut tag_acc = _FHE_LLM_mul_const(module, tags[0], &weights[0]);
    for i in 1..tags.len() {
        let term = _FHE_LLM_mul_const(module, tags[i], &weights[i]);
        tag_acc = _FHE_LLM_add(module, &tag_acc, &term);
    }
...
/// modulus is 256.
fn plaintext_modulus(params: &FHE_LLMParams) -> i64 {
...
///
/// * `module` - Backend module.
/// * `key` - The user's FHE_LLM secret key (for decryption).
/// * `params` - FHE_LLM parameters.
/// * `mac_key` - The user's secret MAC key.
...
pub fn _FHE_LLM_mac_verify<BE: Backend>(
    module: &Module<BE>,
    key: &FHE_LLMKey<BE>,
    params: &FHE_LLMParams,
    mac_key: &MacKey,
...
{
    use crate::encrypt::_FHE_LLM_decrypt;

    let ring_degree = module.n() as usize;
    assert!(
        n_slots <= ring_degree,
        "_FHE_LLM_mac_verify: n_slots ({n_slots}) exceeds ring degree ({ring_degree})"
    );

    let pt_ct = _FHE_LLM_decrypt(module, key, &tagged.ct, params);
    let pt_tag = _FHE_LLM_decrypt(module, key, &tagged.tag, params);
...
pub fn _FHE_LLM_mac_verify_vec<BE: Backend>(
    module: &Module<BE>,
    key: &FHE_LLMKey<BE>,
    params: &FHE_LLMParams,
    mac_key: &MacKey,
    ...
    Scratch<BE>: poulpy_core::ScratchTakeCore<BE>,
{
    ...
    assert!(
        n_slots <= ring_degree,
        "_FHE_LLM_mac_verify_vec: n_slots ({n_slots}) exceeds ring degree ({ring_degree})"
    );
    ...
    for tagged in tagged_vec {
        let result = _FHE_LLM_mac_verify(module, key, params, mac_key, tagged, n_slots, tolerance);
...
mod tests {
    ...
    fn test_mac_tag_and_verify_identity() {
        ...
        let tagged = _FHE_LLM_mac_tag(&module, &mac, &ct);

        // Verify with tight tolerance (mul_const error is at most 4)
        let result = _FHE_LLM_mac_verify(&module, &key, &params, &mac, &tagged, 5, 25);
    ...
    fn test_mac_through_add() {
        ...
        let tagged_a = _FHE_LLM_mac_tag(&module, &mac, &ct_a);
        let tagged_b = _FHE_LLM_mac_tag(&module, &mac, &ct_b);

        // Provider computes addition on both ct and tag
        let tagged_sum = tagged_add(&module, &tagged_a, &tagged_b);

        // User verifies
        let result = _FHE_LLM_mac_verify(&module, &key, &params, &mac, &tagged_sum, 3, 25);
    ...
    fn test_mac_through_mul_const() {
        ...
        // User verifies
        let result = _FHE_LLM_mac_verify(&module, &key, &params, &mac, &tagged_result, 3, 50);
    ...
    fn test_mac_through_dot_product() {
        ...
        // User verifies
        let result = _FHE_LLM_mac_verify(&module, &key, &params, &mac, &tagged_result, 1, 50);
    ...
    fn test_mac_detects_tampered_output() {
        ...
        let tampered_ct = _FHE_LLM_encrypt(&module, &key, &tampered_pt, [5u8; 32], [6u8; 32]);
        ...
        let result = _FHE_LLM_mac_verify(&module, &key, &params, &mac, &tampered_tagged, 3, 25);
    ...
    fn test_mac_detects_wrong_weights() {
        ...
        let honest_tag_result = _FHE_LLM_mul_const(&module, &tagged.tag, &[3]);

        // Dishonest computation: multiply by 5 (wrong weights!)
        let dishonest_ct_result = _FHE_LLM_mul_const(&module, &tagged.ct, &[5]);
        ...
        let result = _FHE_LLM_mac_verify(&module, &key, &params, &mac, &mismatched, 3, 25);
    ...
    fn test_mac_through_chained_operations() {
        ...
        // Verify the chain
        let result = _FHE_LLM_mac_verify(&module, &key, &params, &mac, &step3, 3, 80);
    ...
    fn test_mac_verify_rejects_too_many_slots() {
        ...
        _FHE_LLM_mac_verify(&module, &key, &params, &mac, &tagged, module.n() as usize + 1, 25);
    }
    ...
    fn test_mac_matmul_single_ct() {
        ...
        // Verify each output
        let result = _FHE_LLM_mac_verify_vec(&module, &key, &params, &mac, &tagged_results, 2, 50);
