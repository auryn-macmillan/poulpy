...
//! weights from the safetensors format and converting them into CHIMERA's
...
//! CHIMERA's weight matrices use the convention `[in_features, out_features]` for
...
pub struct LoaderConfig {
    ...
    /// Default: `true` (PyTorch convention → CHIMERA convention).
    pub transpose: bool,
...
///
/// ```ignore
/// use poulpy_chimera::model_loader::{LoaderConfig, load_layer};
/// use poulpy_chimera::params::ModelDims;
///
...
///
/// ```ignore
/// use poulpy_chimera::model_loader::{LoaderConfig, load_model_from_file};
/// use poulpy_chimera::params::ModelDims;
///
/// let dims = ModelDims::dense_7b();
/// let config = LoaderConfig::llama(dims);
