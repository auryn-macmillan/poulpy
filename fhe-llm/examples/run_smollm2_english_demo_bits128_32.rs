...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Bits128 security, Fp32, 2048‑level LUTs, frequent bootstrap enabled, truncation to 32 dims
    let config = InferenceConfig {
        security: SecurityLevel::Bits128,
        precision: Precision::Fp32,
        num_layers: Some(1),     // keep single layer for tractability
        trunc_d_model: Some(32), // reduce hidden dimension
        trunc_d_ffn: Some(64),   // keep FFN dimension proportional (d_ffn ~ 2*d_model)
        num_heads: Some(2),      // d_head = 16
        num_kv_heads: Some(1),
        softmax_strategy: SoftmaxStrategy::ReluSquared,
        apply_final_norm: false,
        fhe_frequent_bootstrap: true,
        // 2048‑level LUTs (log_msg_mod=11)
