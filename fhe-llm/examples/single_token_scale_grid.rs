...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let remap = parse_usize_list(args.next(), &[2, 3, 4, 8]);
    let qkv = parse_usize_list(args.next(), &[4, 8, 10]);
    ...
    let grid = pipeline.sweep_single_token_scale_grid(token_id, &remap, &qkv)?;

    println!("token_id: {}", grid.token_id);
    println!("refreshed: token={} {:?}", grid.refreshed_token_id, grid.refreshed_token_text);
    for row in grid.rows {
        println!(
            "remap={} qkv={}: token={} {:?} | L-inf={:.3} L2={:.3} MAE={:.3} | refreshed_rank={} pred_logit={:.4} refreshed_logit={:.4}",
