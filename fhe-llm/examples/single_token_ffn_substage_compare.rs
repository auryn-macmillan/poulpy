...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mode = std::env::args().nth(2).unwrap_or_else(|| "hybrid".to_string());
    ...
    let rows = match mode.as_str() {
        "poly" => pipeline.compare_ffn_substages_with_residual_refresh(token_id),
        "hybrid" | "lut" => pipeline.compare_ffn_substages_with_residual_refresh_lut_gate(token_id),
