//! RLWE encryption and decryption for FHE_LLM.
//!
//! Wraps `poulpy-core`'s GLWE secret-key encryption with FHE_LLM-specific
...
/// Key material for FHE_LLM encryption/decryption.
///
...
///
pub struct FHE_LLMKey<BE: Backend> {
...
impl<BE: Backend> Drop for FHE_LLMKey<BE> {
    fn drop(&mut self) {
        self.secret.fill_zero();
    }
}

impl<BE: Backend> FHE_LLMKey<BE>
where
    Module<BE>: ModuleN + GLWESecretPreparedFactory<BE>,
{
    /// Generates a new FHE_LLM key pair from the given parameters.
    ///
    /// # Arguments
    ///
    ...
    /// from poulpy-core. See [`FHE_LLMParams`] for details on the base2k cascade.
    pub fn generate(module: &Module<BE>, params: &FHE_LLMParams, seed: [u8; 32]) -> Self {
...
pub struct FHE_LLMEvalKey<BE: Backend> {
...
impl<BE: Backend> FHE_LLMEvalKey<BE>
where
    Module<BE>: ModuleN
...
/// Encrypts a GLWE plaintext under FHE_LLM parameters.
///
...
///
/// * `module` - The backend module.
/// * `key` - FHE_LLM key material.
/// * `pt` - The plaintext to encrypt.
/// * `seed_a` - Seed for mask sampling.
/// * `seed_e` - Seed for error sampling.
pub fn _FHE_LLM_encrypt<BE: Backend>(
    module: &Module<BE>,
    key: &FHE_LLMKey<BE>,
...
/// Decrypts a GLWE ciphertext under FHE_LLM parameters.
///
...
/// * `_params` - FHE_LLM parameter set (reserved for future use).
pub fn _FHE_LLM_decrypt<BE: Backend>(
    module: &Module<BE>,
    key: &FHE_LLMKey<BE>,
    ct: &GLWE<Vec<u8>>,
