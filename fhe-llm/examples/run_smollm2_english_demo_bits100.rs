...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // High‑precision configuration (Bits100 security, FP16, 512‑level LUTs, no frequent bootstrap)
    let config = InferenceConfig {
        ...
        // Use 512‑level LUTs to reduce quantisation error (log_msg_mod=9)
        fhe_silu_log_msg_mod: Some(9),
        fhe_identity_log_msg_mod: Some(9),
