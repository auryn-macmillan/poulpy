...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let compress_bits = args.next().map(|s| s.parse::<usize>()).transpose()?.unwrap_or(3);
    ...
    let result = pipeline.compare_prompt_compressed_lut_gate_shadow_layers(&prompt, &variant, fractional_bits, compress_bits)?;

    println!("prompt: {:?}", result.prompt);
    println!("tokenized_prompt_len: {}", result.prompt_tokens.len());
    println!(
        "gate_variant: {} fractional_bits={} compress_bits={}",
        result.gate_variant, result.fractional_bits, result.compress_bits
    );
