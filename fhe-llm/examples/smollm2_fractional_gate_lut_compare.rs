...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = pipeline.compare_prompt_fractional_gate_lut(&prompt, fractional_bits)?;

    println!("prompt: {:?}", result.prompt);
    println!("tokenized_prompt_len: {}", result.prompt_tokens.len());
    println!("fractional_bits: {}", result.fractional_bits);
    println!(
        "baseline token: {} {:?}",
        result.baseline_token_id, result.baseline_token_text
    );
    println!("trial token: {} {:?}", result.trial_token_id, result.trial_token_text);
    println!(
        "hidden error: L-inf={:.6} L2={:.6} MAE={:.6}",
        result.hidden_linf, result.hidden_l2, result.hidden_mae
    );
