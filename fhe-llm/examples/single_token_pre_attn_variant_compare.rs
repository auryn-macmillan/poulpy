...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for row in pipeline.compare_pre_attn_norm_variants_for_token(token_id) {
        println!("{}: L-inf={:.3} L2={:.3} MAE={:.3}", row.stage, row.linf, row.l2, row.mae);
    }
    println!("without gamma:");
    for row in pipeline.compare_pre_attn_norm_variants_without_gamma_for_token(token_id) {
        println!("{}: L-inf={:.3} L2={:.3} MAE={:.3}", row.stage, row.linf, row.l2, row.mae);
    }
