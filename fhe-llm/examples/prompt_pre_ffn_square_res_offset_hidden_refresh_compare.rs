...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let hidden_refresh_decode = args.next().map(|s| s.parse::<u32>()).transpose()?.unwrap_or(2);
    ...
    let comparison = pipeline.compare_prompt_fhe_semantics_with_pre_ffn_square_res_offset_and_hidden_refresh(
        &prompt,
        attn_decode,
        residual_decode,
        input_shift,
        square_res_offset,
        hidden_refresh_decode,
    )?;

    println!("prompt: {:?}", comparison.prompt);
    println!("prompt_len: {}", comparison.prompt_tokens.len());
    println!(
        "config: attn_decode={} residual_decode={} input_shift={} square_res_offset={} hidden_refresh_decode={}",
        attn_decode, residual_decode, input_shift, square_res_offset, hidden_refresh_decode
    );
    println!(
        "refreshed token: {} {:?}",
