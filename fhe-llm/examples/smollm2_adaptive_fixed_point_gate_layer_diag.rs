...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let diag = pipeline.diagnose_prompt_adaptive_fixed_point_gate_layers(&prompt, fractional_bits)?;

    println!("prompt: {:?}", diag.prompt);
    println!("tokenized_prompt_len: {}", diag.prompt_tokens.len());
    println!(
        "message_domain: log_message_modulus={} range=[{}, {}] fractional_bits={}",
        diag.log_message_modulus, diag.message_range_min, diag.message_range_max, diag.fractional_bits
    );
