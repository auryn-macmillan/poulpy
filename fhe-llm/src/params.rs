...
//!
//! This module defines the parameter sets for FHE_LLM at different security
...
pub enum Precision {
    ...
    Fp16,
    /// 32-bit floating point (encoded as fixed-point). Suitable for high-precision activations.
    Fp32,
}

/// Complete parameter set for a FHE_LLM instance.
///
/// All fields are determined by the security level and precision mode.
/// Users should construct this via [`FHE_LLMParams::new`] rather than
...
///
/// Ciphertexts produced by `_FHE_LLM_encrypt` use `in_base2k`, NOT `base2k`.
/// This matches the pattern from poulpy-core's tensor test.
#[derive(Clone, Debug)]
pub struct FHE_LLMParams {
...
impl ModelDims {
    ...
    /// SmolLM2-135M-Instruct model dimensions.
    pub fn smollm2_135m_instruct() -> Self {
        ModelDims {
            d_model: 576,
            d_head: 64,
            n_heads: 9,
...
mod tests {
    ...
    fn test_params_100bit_fp16() {
        let params = FHE_LLMParams::new(SecurityLevel::Bits100, Precision::Fp16);
