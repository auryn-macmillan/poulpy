...
use crate::activations::{
    apply_poly_activation, apply_poly_activation_with_encoding_scale, _FHE_LLM_ct_mul, gelu_poly_approx, silu_poly_approx,
    silu_poly_approx_narrow, squared_relu_approx, PolyApprox,
};
use crate::arithmetic::{_FHE_LLM_add, _FHE_LLM_align_layout, _FHE_LLM_matmul_single_ct};
use crate::attention::{
    _FHE_LLM_apply_softmax, _FHE_LLM_attention_context, _FHE_LLM_attention_score, _FHE_LLM_multi_head_attention_vec,
    _FHE_LLM_output_project, _FHE_LLM_qkv_project_single, plan_attention, AttentionConfig, AttentionPlan, AttentionWeights,
    SoftmaxStrategy,
};
use crate::bootstrapping::{_FHE_LLM_bootstrap, needs_bootstrap, BootstrappingConfig, FHE_LLMBootstrapKeyPrepared};
use crate::encrypt::FHE_LLMEvalKey;
use crate::layernorm::{_FHE_LLM_rms_norm, _FHE_LLM_rms_norm_vec, plan_layernorm, LayerNormConfig, LayerNormPlan};
use crate::noise::NoiseTracker;
use crate::params::{FHE_LLMParams, ModelDims};
    ...
    Scratch<BE>: ScratchAvailable,
{
    x_cts
        .iter()
        .map(|ct| crate::arithmetic::_FHE_LLM_mul_const_scaled(module, ct, &[1], 1))
        .collect()
}
...
pub fn plan_forward_pass(config: &TransformerBlockConfig, num_layers: usize, params: &FHE_LLMParams) -> ForwardPassPlan {
...
pub fn default_block_config(dims: ModelDims, params: FHE_LLMParams) -> TransformerBlockConfig {
...
///    [`_FHE_LLM_matmul_single_ct`], producing one intermediate ciphertext per
///    hidden dimension.
///
...
pub fn _FHE_LLM_ffn_standard<BE: Backend>(
    module: &Module<BE>,
    ...
    Scratch<BE>: ScratchTakeCore<BE> + ScratchAvailable,
{
    ...
    let h = _FHE_LLM_matmul_single_ct(module, x, &w1_rows);
    ...
    // _FHE_LLM_mul_const(h'_j, [w]), then accumulate with _FHE_LLM_add.
    //
    // The activated intermediates carry an extra *2^COEFF_SCALE_BITS factor
    // from apply_poly_activation. Use _FHE_LLM_dot_product_scaled to compensate.
    ...
    for w2_row in &weights.w2 {
        ...
        let dot = crate::arithmetic::_FHE_LLM_dot_product_scaled(module, &h_act, &w2_vecs, crate::activations::COEFF_SCALE_BITS);
        outputs.push(dot);
    }
...
pub fn _FHE_LLM_ffn_swiglu<BE: Backend>(
    module: &Module<BE>,
    eval_key: &FHE_LLMEvalKey<BE>,
    x: &GLWE<Vec<u8>>,
    weights: &FFNWeights,
    ...
    Scratch<BE>: ScratchTakeCore<BE> + ScratchAvailable,
{
    ...
    let gate = _FHE_LLM_matmul_single_ct(module, x, &w_gate_rows);

    // Phase 2: Up projection — up_j = W_up[j] · x
    let w_up_rows: Vec<Vec<i64>> = w_up.iter().map(|r| r.clone()).collect();
    let up = _FHE_LLM_matmul_single_ct(module, x, &w_up_rows);
    ...
    for j in 0..d_ffn {
        ...
        let up_projected = if up[j].base2k() == gate_activated.base2k() {
            ...
            _FHE_LLM_align_layout(module, &up[j], &gate_layout)
        };

        // Element-wise product: SiLU(gate_j) ⊙ up_j
        let hj = _FHE_LLM_ct_mul(module, eval_key, &gate_activated, &up_projected);
        h.push(hj);
    }
    ...
    // Use _FHE_LLM_dot_product_scaled to compensate.
    let mut outputs = Vec::with_capacity(weights.w2.len());
    for w2_row in &weights.w2 {
        assert_eq!(w2_row.len(), h.len(), "W_down row length must match hidden dimension (d_ffn)");
        let w2_vecs: Vec<Vec<i64>> = w2_row.iter().map(|&w| vec![w]).collect();
        let dot = crate::arithmetic::_FHE_LLM_dot_product_scaled(module, &h, &w2_vecs, crate::activations::COEFF_SCALE_BITS);
        outputs.push(dot);
    }
...
///
/// Calls [`_FHE_LLM_ffn_standard`] for `FFNConfig::Standard` or
/// [`_FHE_LLM_ffn_swiglu`] for `FFNConfig::SwiGLU`.
pub fn _FHE_LLM_ffn<BE: Backend>(
    module: &Module<BE>,
    eval_key: &FHE_LLMEvalKey<BE>,
    x: &GLWE<Vec<u8>>,
    weights: &FFNWeights,
...
{
    match config {
        FFNConfig::Standard { activation } => _FHE_LLM_ffn_standard(module, eval_key, x, weights, activation),
        FFNConfig::SwiGLU => _FHE_LLM_ffn_swiglu(module, eval_key, x, weights),
    }
}
...
pub fn _FHE_LLM_residual_add<BE: Backend>(module: &Module<BE>, a: &[GLWE<Vec<u8>>], b: &[GLWE<Vec<u8>>]) -> Vec<GLWE<Vec<u8>>>
where
    Module<BE>: GLWEAdd,
{
    assert_eq!(a.len(), b.len(), "residual add: vectors must have the same length");
    a.iter().zip(b.iter()).map(|(ai, bi)| _FHE_LLM_add(module, ai, bi)).collect()
}
...
/// ## Computation Flow
///
/// 1. **Pre-attention RMSNorm**: Normalises the input via `_FHE_LLM_rms_norm`.
...
/// and backward-compatibility tests. Use [`_FHE_LLM_transformer_block_vec`] for
/// model-faithful transformer evaluation.
///
/// # Panics
///
/// Panics if norm configurations are not set to RMSNorm mode.
#[deprecated(note = "legacy toy-only API; use _FHE_LLM_transformer_block_vec instead")]
pub fn _FHE_LLM_transformer_block<BE: Backend>(
    module: &Module<BE>,
    eval_key: &FHE_LLMEvalKey<BE>,
    ct_x: &GLWE<Vec<u8>>,
    ...
    Scratch<BE>: ScratchTakeCore<BE> + ScratchAvailable,
{
    ...
    let normed_pre_attn = _FHE_LLM_rms_norm(module, eval_key, ct_x, &pre_attn_norm_cfg);
    ...
    let attn_out = {
        ...
        let (ct_q0, ct_k0, ct_v0) = _FHE_LLM_qkv_project_single(
            module,
            &normed_pre_attn,
        ...
        let score0 = _FHE_LLM_attention_score(module, eval_key, &ct_q0, &ct_k0, 0);
        let attn_wts0 = _FHE_LLM_apply_softmax(module, eval_key, &[score0], &config.attention.softmax_approx);
        let context0 = _FHE_LLM_attention_context(module, eval_key, &attn_wts0, &[ct_v0]);
        let head_out_vec0 = _FHE_LLM_output_project(module, &context0, &[weights.attention.w_o[0].clone()]);
        ...
        for h in 1..n_heads {
            let (ct_q_h, ct_k_h, ct_v_h) = _FHE_LLM_qkv_project_single(
                module,
                &normed_pre_attn,
            ...
            let score_h = _FHE_LLM_attention_score(module, eval_key, &ct_q_h, &ct_k_h, 0);
            let attn_wts_h = _FHE_LLM_apply_softmax(module, eval_key, &[score_h], &config.attention.softmax_approx);
            let context_h = _FHE_LLM_attention_context(module, eval_key, &attn_wts_h, &[ct_v_h]);
            let head_out_vec_h = _FHE_LLM_output_project(module, &context_h, &[weights.attention.w_o[h].clone()]);
            ...
            let acc_proj = if acc.base2k() == head_out_h.base2k() && acc.k() == head_out_h.k() {
                ...
                _FHE_LLM_align_layout(module, &acc, &head_layout)
            };
            acc = _FHE_LLM_add(module, &acc_proj, &head_out_h);
        }
    ...
    let residual_1 = if config.residual {
        ...
        let ct_x_proj = if ct_x.base2k() == attn_out.base2k() && ct_x.k() == attn_out.k() {
            ...
            _FHE_LLM_align_layout(module, ct_x, &ct_x_layout)
        };
        _FHE_LLM_add(module, &ct_x_proj, &attn_out)
    } else {
        attn_out
    };
    ...
    let normed_pre_ffn = _FHE_LLM_rms_norm(module, eval_key, &residual_1, &pre_ffn_norm_cfg);
    ...
    // -----------------------------------------------------------------------
    let ffn_out_vec = _FHE_LLM_ffn(module, eval_key, &normed_pre_ffn, &weights.ffn, &config.ffn);
    ...
    if config.residual {
        ...
        let res1_proj = if residual_1.base2k() == ffn_out.base2k() && residual_1.k() == ffn_out.k() {
            ...
            _FHE_LLM_align_layout(module, &residual_1, &res1_layout)
        };
        _FHE_LLM_add(module, &res1_proj, &ffn_out)
    } else {
        ffn_out
    }
}
...
/// [`_FHE_LLM_transformer_block`]. Use [`_FHE_LLM_forward_pass_vec`] for the
/// vector representation used by real-model inference.
///
/// # Panics
///
/// Panics if `layer_weights.len()` does not match `num_layers`.
#[deprecated(note = "legacy toy-only API; use _FHE_LLM_forward_pass_vec instead")]
#[allow(deprecated)]
pub fn _FHE_LLM_forward_pass<BE: Backend>(
    module: &Module<BE>,
    eval_key: &FHE_LLMEvalKey<BE>,
    ct_x: &GLWE<Vec<u8>>,
    config: &TransformerBlockConfig,
    ...
    Scratch<BE>: ScratchTakeCore<BE> + ScratchAvailable,
{
    ...
    for (layer_idx, weights) in layer_weights.iter().enumerate() {
        current = _FHE_LLM_transformer_block(module, eval_key, &current, config, weights);
        ...
        //       current = _FHE_LLM_bootstrap(module, eval_key, &current, params);
        //   }
...
/// * `config` - Block configuration (shared across all layers).
/// * `layer_weights` - Per-layer weight matrices.
/// * `params` - FHE_LLM parameter set (used for noise budget calculation).
...
#[allow(deprecated)]
pub fn _FHE_LLM_forward_pass_with_bootstrap<BE: Backend>(
    module: &Module<BE>,
    eval_key: &FHE_LLMEvalKey<BE>,
    ct_x: &GLWE<Vec<u8>>,
    config: &TransformerBlockConfig,
    ...
    for (layer_idx, weights) in layer_weights.iter().enumerate() {
        ...
        if needs_bootstrap(&tracker, params, bootstrap_config) {
            let bsk = bsk_prepared.expect(
                "_FHE_LLM_forward_pass_with_bootstrap: bootstrapping triggered \
            ...
            if bootstrap_config.max_bootstraps_per_pass == 0 || bootstrap_count < bootstrap_config.max_bootstraps_per_pass {
                current = _FHE_LLM_bootstrap(module, &current, &mut tracker, params, bsk, bootstrap_config);
                bootstrap_count += 1;
            }
...
///
/// A vector of ciphertexts, one per output dimension (one per row of `w2`).
pub fn _FHE_LLM_ffn_standard_vec<BE: Backend>(
    module: &Module<BE>,
    eval_key: &FHE_LLMEvalKey<BE>,
    x_cts: &[GLWE<Vec<u8>>],
    weights: &FFNWeights,
...
where
    ...
    Scratch<BE>: ScratchTakeCore<BE> + ScratchAvailable,
{
    _FHE_LLM_ffn_standard_vec_scaled(module, eval_key, x_cts, weights, activation, 0)
}

pub fn _FHE_LLM_ffn_standard_vec_scaled<BE: Backend>(
    module: &Module<BE>,
    eval_key: &FHE_LLMEvalKey<BE>,
    x_cts: &[GLWE<Vec<u8>>],
    weights: &FFNWeights,
    ...
    Scratch<BE>: ScratchTakeCore<BE> + ScratchAvailable,
{
        ...
        (0..d_ffn)
            ...
            .map(|j| {
                ...
                let hj = if input_scale_shift_bits == 0 {
                    crate::arithmetic::_FHE_LLM_dot_product(module, x_cts, &w_vecs)
                } else {
                    crate::arithmetic::_FHE_LLM_dot_product_scaled(module, x_cts, &w_vecs, input_scale_shift_bits)
                };
    ...
    let outputs: Vec<GLWE<Vec<u8>>> = {
        ...
        weights
            ...
            .map(|w2_row| {
                let w2_vecs: Vec<Vec<i64>> = w2_row.iter().map(|&w| vec![w]).collect();
                crate::arithmetic::_FHE_LLM_dot_product_scaled(module, &h_act, &w2_vecs, crate::activations::COEFF_SCALE_BITS)
            })
            .collect()
    };
...
pub fn _FHE_LLM_ffn_swiglu_vec<BE: Backend>(
    module: &Module<BE>,
    eval_key: &FHE_LLMEvalKey<BE>,
    x_cts: &[GLWE<Vec<u8>>],
    weights: &FFNWeights,
...
where
    ...
    Scratch<BE>: ScratchTakeCore<BE> + ScratchAvailable,
{
    _FHE_LLM_ffn_swiglu_vec_scaled(module, eval_key, x_cts, weights, 0)
}

pub fn _FHE_LLM_ffn_swiglu_vec_scaled<BE: Backend>(
    module: &Module<BE>,
    eval_key: &FHE_LLMEvalKey<BE>,
    x_cts: &[GLWE<Vec<u8>>],
    weights: &FFNWeights,
    ...
    Scratch<BE>: ScratchTakeCore<BE> + ScratchAvailable,
{
        ...
        (0..d_ffn)
            ...
            .map(|j| {
                ...
                let gate_j = if input_scale_shift_bits == 0 {
                    crate::arithmetic::_FHE_LLM_dot_product(module, x_cts, &wg_vecs)
                } else {
                    crate::arithmetic::_FHE_LLM_dot_product_scaled(module, x_cts, &wg_vecs, input_scale_shift_bits)
                };

                // Up: up_j = Σ_k W_up[j][k] * x_k
                let wu_vecs: Vec<Vec<i64>> = w_up[j][..d_in].iter().map(|&w| vec![w]).collect();
                let up_j = if input_scale_shift_bits == 0 {
                    crate::arithmetic::_FHE_LLM_dot_product(module, x_cts, &wu_vecs)
                } else {
                    crate::arithmetic::_FHE_LLM_dot_product_scaled(module, x_cts, &wu_vecs, input_scale_shift_bits)
                };
                ...
                let up_proj = if up_j.base2k() == gate_activated.base2k() {
                    ...
                    _FHE_LLM_align_layout(module, &up_j, &gate_layout)
                };

                // Element-wise: SiLU(gate_j) ⊙ up_j
                let h_j = if input_scale_shift_bits == 0 {
                    _FHE_LLM_ct_mul(module, eval_key, &gate_activated, &up_proj)
                } else {
    ...
    // We use `_FHE_LLM_dot_product_scaled` to compensate, matching the standard
    ...
    let outputs: Vec<GLWE<Vec<u8>>> = {
        ...
        weights
            ...
            .map(|w2_row| {
                let w2_vecs: Vec<Vec<i64>> = w2_row.iter().map(|&w| vec![w]).collect();
                crate::arithmetic::_FHE_LLM_dot_product_scaled(module, &h, &w2_vecs, crate::activations::COEFF_SCALE_BITS)
            })
            .collect()
    };
...
pub fn _FHE_LLM_ffn_vec<BE: Backend>(
    module: &Module<BE>,
    eval_key: &FHE_LLMEvalKey<BE>,
    x_cts: &[GLWE<Vec<u8>>],
    weights: &FFNWeights,
...
where
    ...
    Scratch<BE>: ScratchTakeCore<BE> + ScratchAvailable,
{
    _FHE_LLM_ffn_vec_scaled(module, eval_key, x_cts, weights, config, 0)
}

pub fn _FHE_LLM_ffn_vec_scaled<BE: Backend>(
    module: &Module<BE>,
    eval_key: &FHE_LLMEvalKey<BE>,
    x_cts: &[GLWE<Vec<u8>>],
    weights: &FFNWeights,
    ...
    Scratch<BE>: ScratchTakeCore<BE> + ScratchAvailable,
{
    match config {
        FFNConfig::Standard { activation } => {
            _FHE_LLM_ffn_standard_vec_scaled(module, eval_key, x_cts, weights, activation, input_scale_shift_bits)
        }
        FFNConfig::SwiGLU => _FHE_LLM_ffn_swiglu_vec_scaled(module, eval_key, x_cts, weights, input_scale_shift_bits),
    }
}

/// Evaluates a complete transformer block in the vector representation.
///
/// This is the vector-representation variant of [`_FHE_LLM_transformer_block`].
...
pub fn _FHE_LLM_transformer_block_vec<BE: Backend>(
    module: &Module<BE>,
    eval_key: &FHE_LLMEvalKey<BE>,
    x_cts: &[GLWE<Vec<u8>>],
    config: &TransformerBlockConfig,
    ...
    Scratch<BE>: ScratchTakeCore<BE> + ScratchAvailable,
{
    ...
    let normed_pre_attn = _FHE_LLM_rms_norm_vec(module, eval_key, x_cts, &pre_attn_norm_cfg);
    let pre_attn_norm_ms = t0.elapsed().as_millis();

    // Step 2: Multi-head attention (vector representation)
    let t1 = std::time::Instant::now();
    let attn_out = _FHE_LLM_multi_head_attention_vec(module, eval_key, &normed_pre_attn, &weights.attention, &config.attention);
    ...
    let normed_pre_ffn = _FHE_LLM_rms_norm_vec(module, eval_key, &residual_1_for_norm, &pre_ffn_norm_cfg);
    ...
    Scratch<BE>: ScratchAvailable,
{
    ...
    for i in 0..d {
        let (ai_proj, bi_ref) = if a[i].base2k() == b[i].base2k() && a[i].k() == b[i].k() {
            ...
            let projected = _FHE_LLM_align_layout(module, &a[i], &target);
            (projected, &b[i])
        };
        result.push(_FHE_LLM_add(module, &ai_proj, bi_ref));
    }
...
pub fn _FHE_LLM_forward_pass_vec<BE: Backend>(
    module: &Module<BE>,
    eval_key: &FHE_LLMEvalKey<BE>,
    x_cts: &[GLWE<Vec<u8>>],
    config: &TransformerBlockConfig,
    ...
    Scratch<BE>: ScratchTakeCore<BE> + ScratchAvailable,
{
    ...
    for weights in layer_weights {
        current = _FHE_LLM_transformer_block_vec(module, eval_key, &current, config, weights);
    }
...
pub struct ForwardPassConfig {
    ...
    /// FHE_LLM parameters (needed for noise budget calculations).
    pub params: FHE_LLMParams,
...
/// - The final [`NoiseTracker`] state (useful for diagnostics).
pub fn _FHE_LLM_forward_pass_vec_full<BE: Backend>(
    module: &Module<BE>,
    eval_key: &FHE_LLMEvalKey<BE>,
    x_cts: &[GLWE<Vec<u8>>],
    ...
    Scratch<BE>: ScratchTakeCore<BE> + ScratchAvailable,
{
    ...
    for (layer_idx, weights) in layer_weights.iter().enumerate() {
        ...
        if needs_bootstrap(&tracker, &config.params, &config.bootstrap) {
            let bsk = bsk_prepared.expect(
                "_FHE_LLM_forward_pass_vec_full: bootstrapping triggered \
                 but no bootstrap key provided (bsk_prepared is None)",
            );
            ...
            if config.bootstrap.max_bootstraps_per_pass == 0 || bootstrap_count < config.bootstrap.max_bootstraps_per_pass {
                ...
                current = current
                    ...
                    .map(|ct| {
                        ...
                        _FHE_LLM_bootstrap(module, ct, &mut t, &config.params, bsk, &config.bootstrap)
                    })
                    .collect();
        ...
        // --- Evaluate transformer block ---
        current = _FHE_LLM_transformer_block_vec(module, eval_key, &current, &config.block, weights);
    ...
    if let Some(ref gamma) = config.final_norm_gamma {
        ...
        current = _FHE_LLM_rms_norm_vec(module, eval_key, &current, &norm_cfg);
    }
...
mod tests {
    ...
    fn test_ffn_standard_homomorphic() {
        ...
        use crate::encrypt::{_FHE_LLM_encrypt, FHE_LLMEvalKey, FHE_LLMKey};
        ...
        let key = FHE_LLMKey::generate(&module, &params, [1u8; 32]);
        let eval_key = FHE_LLMEvalKey::generate(&module, &key, &params, [4u8; 32], [5u8; 32]);

        let vals: Vec<i8> = vec![3];
        let pt = encode_int8(&module, &params, &vals);
        let ct = _FHE_LLM_encrypt(&module, &key, &pt, [2u8; 32], [3u8; 32]);
    ...
    #[test]
    fn test__FHE_LLM_ffn_dispatch() {
        // Test the _FHE_LLM_ffn dispatch function routes correctly.
        use crate::encoding::encode_int8;
        use crate::encrypt::{_FHE_LLM_encrypt, FHE_LLMEvalKey, FHE_LLMKey};
        use poulpy_hal::api::ModuleNew;
        ...
        let result_std = _FHE_LLM_ffn(&module, &eval_key, &ct, &std_weights, &config_std);
        ...
        let result_swiglu = _FHE_LLM_ffn(&module, &eval_key, &ct, &swiglu_weights, &config_swiglu);
        assert_eq!(result_swiglu.len(), 1);
    }

    #[test]
    fn test_residual_add() {
        // Test _FHE_LLM_residual_add on a pair of ciphertext vectors.
        use crate::encoding::{decode_int8, encode_int8};
        use crate::encrypt::{_FHE_LLM_decrypt, _FHE_LLM_encrypt, FHE_LLMKey};
        use poulpy_hal::api::ModuleNew;
        ...
        let ct_a = _FHE_LLM_encrypt(&module, &key, &pt_a, [2u8; 32], [3u8; 32]);
        let ct_b = _FHE_LLM_encrypt(&module, &key, &pt_b, [4u8; 32], [5u8; 32]);

        let result = _FHE_LLM_residual_add(&module, &[ct_a], &[ct_b]);
        assert_eq!(result.len(), 1);

        let pt_dec = _FHE_LLM_decrypt(&module, &key, &result[0], &params);
