//! Benchmark harness for FHE_LLM transformer operations.
//!
...
use poulpy_FHE_LLM::params::*;
use poulpy_FHE_LLM::encoding::*;
use poulpy_FHE_LLM::encrypt::*;
use poulpy_FHE_LLM::arithmetic::*;
use poulpy_FHE_LLM::activations::*;
use poulpy_FHE_LLM::layernorm::*;
use poulpy_FHE_LLM::transformer::*;
use poulpy_FHE_LLM::lut::*;
use poulpy_FHE_LLM::noise::*;
...
fn bench_encoding(c: &mut Criterion) {
    ...
    c.bench_function("FHE_LLM/encode_int8", |b| {
        b.iter(|| encode_int8(&module, &params, black_box(&values)))
    });

    let fp_values: Vec<f32> = (0..params.slots).map(|i| (i as f32) / 1000.0).collect();
    let fp_params = FHE_LLMParams::new(SecurityLevel::Bits80, Precision::Fp16);

    c.bench_function("FHE_LLM/encode_fp16", |b| {
...
fn bench_encrypt_decrypt(c: &mut Criterion) {
    ...
    c.bench_function("FHE_LLM/encrypt", |b| {
        b.iter(|| _FHE_LLM_encrypt(&module, &key, black_box(&pt), [1u8; 32], [2u8; 32]))
    });

    let ct = _FHE_LLM_encrypt(&module, &key, &pt, [1u8; 32], [2u8; 32]);

    c.bench_function("FHE_LLM/decrypt", |b| {
        b.iter(|| _FHE_LLM_decrypt(&module, &key, black_box(&ct), &params))
    });
}
...
fn bench_arithmetic(c: &mut Criterion) {
    ...
    c.bench_function("FHE_LLM/add", |b| {
        b.iter(|| _FHE_LLM_add(&module, black_box(&ct_a), black_box(&ct_b)))
    });

    c.bench_function("FHE_LLM/sub", |b| {
        b.iter(|| _FHE_LLM_sub(&module, black_box(&ct_a), black_box(&ct_b)))
    });
}

fn bench_activations(c: &mut Criterion) {
    c.bench_function("FHE_LLM/gelu_poly_eval", |b| {
    ...
    c.bench_function("FHE_LLM/gelu_lut_eval", |b| {
...
fn bench_planning(c: &mut Criterion) {
    c.bench_function("FHE_LLM/plan_dense_7b", |b| {
    ...
    c.bench_function("FHE_LLM/plan_moe_40b", |b| {
...
fn bench_layernorm(c: &mut Criterion) {
    c.bench_function("FHE_LLM/rmsnorm_plaintext_4096", |b| {
...
fn bench_noise_tracking(c: &mut Criterion) {
    c.bench_function("FHE_LLM/noise_estimate_layer", |b| {
        let params = FHE_LLMParams::new(SecurityLevel::Bits128, Precision::Int8);
...
/// Benchmarks ciphertext-plaintext multiplication (_FHE_LLM_mul_const).
fn bench_mul_const(c: &mut Criterion) {
    ...
    c.bench_function("FHE_LLM/mul_const_scalar", |b| {
        b.iter(|| _FHE_LLM_mul_const(&module, black_box(&ct), black_box(&weight)))
    });
}
...
fn bench_ct_ct_mul(c: &mut Criterion) {
    ...
    c.bench_function("FHE_LLM/ct_ct_mul", |b| {
        b.iter(|| _FHE_LLM_ct_mul(&module, &eval_key, black_box(&ct), black_box(&ct)))
    });
}
...
fn bench_poly_activation_fhe(c: &mut Criterion) {
    ...
    c.bench_function("FHE_LLM/activation_sqrelu_fhe", |b| {
    ...
    c.bench_function("FHE_LLM/activation_gelu_fhe", |b| {
...
fn bench_matmul_fhe(c: &mut Criterion) {
    ...
    c.bench_function("FHE_LLM/matmul_single_ct_4rows", |b| {
        b.iter(|| _FHE_LLM_matmul_single_ct(&module, black_box(&ct), black_box(&weight_rows)))
    });
}
...
fn bench_ffn_standard_fhe(c: &mut Criterion) {
    ...
    c.bench_function("FHE_LLM/ffn_standard_d1h2", |b| {
        b.iter(|| {
            _FHE_LLM_ffn_standard(
                &module,
                &eval_key,
                black_box(&ct),
                black_box(&weights),
...
fn bench_matmul_d128(c: &mut Criterion) {
    ...
    c.bench_function("FHE_LLM/matmul_d128", |b| {
        b.iter(|| _FHE_LLM_matmul_single_ct(&module, black_box(&ct), black_box(&weight_rows)))
    });
}
...
fn bench_mul_const_d128(c: &mut Criterion) {
    ...
    c.bench_function("FHE_LLM/mul_const_128coeff", |b| {
        b.iter(|| _FHE_LLM_mul_const(&module, black_box(&ct), black_box(&weight)))
    });
}
...
fn bench_ct_ct_mul_d128(c: &mut Criterion) {
    ...
    c.bench_function("FHE_LLM/ct_ct_mul_d128", |b| {
        b.iter(|| _FHE_LLM_ct_mul(&module, &eval_key, black_box(&ct), black_box(&ct)))
    });
}
...
fn bench_ffn_d4h8(c: &mut Criterion) {
    ...
    c.bench_function("FHE_LLM/ffn_standard_d4h8", |b| {
        b.iter(|| {
            _FHE_LLM_ffn_standard(
                &module,
                &eval_key,
...
fn bench_security_sweep_encrypt(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("FHE_LLM/sweep_encrypt");
...
fn bench_security_sweep_decrypt(c: &mut Criterion) {
    use criterion::BenchmarkId;
    let mut group = c.benchmark_group("FHE_LLM/sweep_decrypt");
...
fn bench_security_sweep_add(c: &mut Criterion) {
    use criterion::BenchmarkId;
    let mut group = c.benchmark_group("FHE_LLM/sweep_add");
...
fn bench_security_sweep_mul_const(c: &mut Criterion) {
    use criterion::BenchmarkId;
    let mut group = c.benchmark_group("FHE_LLM/sweep_mul_const");
...
fn bench_security_sweep_ct_ct_mul(c: &mut Criterion) {
    use criterion::BenchmarkId;
    let mut group = c.benchmark_group("FHE_LLM/sweep_ct_ct_mul");
    ...
    for (label, security) in [
        ...
        group.bench_with_input(BenchmarkId::new("ct_ct_mul", label), &(), |b, _| {
            b.iter(|| {
                use poulpy_FHE_LLM::activations::_FHE_LLM_ct_mul;
                _FHE_LLM_ct_mul(&module, &eval_key, black_box(&ct), black_box(&ct))
            })
        });
    }
    group.finish();
