/// Full SmolLM2 dimensions validation at reduced layer count.
///
/// Validates d=576, d_ffn=1536 at 2-4 layers to confirm full dimensions work.
/// Full 24-layer test estimated at 2-3 hours.
use std::process;
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let n_layers: usize = args.get(1).and_then(|s| s.parse().ok()).unwrap_or(4); // 2-4 layers for validation
    ...
    eprintln!("=== Full SmolLM2 Dimensions Validation (d=576, layers={n_layers}) ===");
