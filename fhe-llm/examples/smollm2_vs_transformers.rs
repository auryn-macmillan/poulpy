...
fn parse_mode(arg: Option<String>) -> &'static str {
    match arg.as_deref() {
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = match mode {
        "refreshed" => pipeline.compare_prompt_refreshed_against_transformers_trace(&prompt, TRACE_PATH)?,
        _ => pipeline.compare_prompt_against_transformers_trace(&prompt, TRACE_PATH)?,
    };
