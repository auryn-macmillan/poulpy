...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let gate_mode = args.next().unwrap_or_else(|| "narrow".to_string());
    let narrow = match gate_mode.as_str() {
        "narrow" => true,
        "wide" => false,
    ...
    let result = pipeline.compare_prompt_exact_attn_poly_gate_shadow_first_layer_sublayers(&prompt, TRACE_PATH, narrow)?;

    println!("prompt: {:?}", user_prompt);
    println!("tokenized_prompt_len: {}", result.prompt_tokens.len());
    println!("gate_mode: {}", gate_mode);
