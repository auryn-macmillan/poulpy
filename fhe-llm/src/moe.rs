...
//! FHE_LLM supports two routing strategies:
//!
...
use crate::arithmetic::{_FHE_LLM_add, _FHE_LLM_matmul_single_ct, _FHE_LLM_mul_const, _FHE_LLM_sub};
use crate::bootstrapping::FHE_LLMBootstrapKeyPrepared;
use crate::encrypt::FHE_LLMEvalKey;
use crate::params::{FHE_LLMParams, ModelDims};
use crate::transformer::{_FHE_LLM_ffn, FFNConfig, FFNWeights};
...
///
/// This is a ct-pt matmul using the existing `_FHE_LLM_matmul_single_ct`
...
pub fn _FHE_LLM_router_logits<BE: Backend>(
    module: &Module<BE>,
    x: &GLWE<Vec<u8>>,
...
{
    _FHE_LLM_matmul_single_ct(module, x, router_weights)
}
...
///
/// This uses the blind rotation pipeline from `_FHE_LLM_bootstrap`, but with
...
/// * `params` - FHE_LLM parameter set.
///
/// # Returns
///
/// A fresh GLWE ciphertext encrypting 0 or 1 (at the blind rotation's
/// output precision `base2k_brk`, `k_res`).
pub fn _FHE_LLM_sign_extract<BE: Backend>(
    module: &Module<BE>,
    ct: &GLWE<Vec<u8>>,
    bsk_prepared: &FHE_LLMBootstrapKeyPrepared<BE>,
    _params: &FHE_LLMParams,
) -> GLWE<Vec<u8>>
where
...
pub fn _FHE_LLM_compare<BE: Backend>(
    module: &Module<BE>,
    a: &GLWE<Vec<u8>>,
    b: &GLWE<Vec<u8>>,
    bsk_prepared: &FHE_LLMBootstrapKeyPrepared<BE>,
...
{
    // Compute diff = a - b
    let diff = _FHE_LLM_sub(module, a, b);
    // Extract sign of the difference
    _FHE_LLM_sign_extract(module, &diff, bsk_prepared, params)
}
...
pub fn _FHE_LLM_cond_swap<BE: Backend>(
    module: &Module<BE>,
...
{
    // diff = b - a
    let diff = _FHE_LLM_sub(module, b, a);

    // Project diff and flag to the output layout (tensor product output) so
    // that all subsequent operations use compatible layouts.
    let diff_proj = crate::arithmetic::_FHE_LLM_align_layout(module, &diff, &eval_key.output_layout);
    let flag_proj = crate::arithmetic::_FHE_LLM_align_layout(module, flag, &eval_key.output_layout);

    // flag_diff = flag * diff (ct × ct multiply via tensor product)
    let flag_diff = crate::activations::_FHE_LLM_ct_mul(module, eval_key, &flag_proj, &diff_proj);

    // Project a and b to the output layout
    let a_proj = crate::arithmetic::_FHE_LLM_align_layout(module, a, &eval_key.output_layout);
    let b_proj = crate::arithmetic::_FHE_LLM_align_layout(module, b, &eval_key.output_layout);

    // new_a = a + flag_diff
    let new_a = _FHE_LLM_add(module, &a_proj, &flag_diff);
    // new_b = b - flag_diff
    let new_b = _FHE_LLM_sub(module, &b_proj, &flag_diff);
...
pub fn _FHE_LLM_topk_sort<BE: Backend>(
    module: &Module<BE>,
    eval_key: &FHE_LLMEvalKey<BE>,
    logits: &[GLWE<Vec<u8>>],
    n_active: usize,
    ...
    Scratch<BE>: ScratchTakeCore<BE> + ScratchAvailable,
{
    ...
    for pass in 0..n_active {
        ...
        for j in (pass + 1..n).rev() {
            ...
            let flag = _FHE_LLM_compare(module, &arr[j], &arr[j - 1], bsk_prepared, params);

            // Conditional swap: if flag=1, swap so the larger moves left
            let (new_left, new_right) = _FHE_LLM_cond_swap(module, eval_key, &flag, &arr[j - 1], &arr[j]);
            arr[j - 1] = new_left;
            arr[j] = new_right;
        }
    }
...
///
/// For FHE_LLM, we use a simpler approach: uniform gating weights (1/n_active)
...
pub fn _FHE_LLM_moe_forward<BE: Backend>(
    module: &Module<BE>,
    eval_key: &FHE_LLMEvalKey<BE>,
    x: &GLWE<Vec<u8>>,
    weights: &MoEWeights,
    ...
    Scratch<BE>: ScratchTakeCore<BE> + ScratchAvailable,
{
    ...
    let logits = _FHE_LLM_router_logits(module, x, &weights.router_weights);
    ...
    match config.strategy {
        RoutingStrategy::EncryptedRouting => {
            ...
            let sorted_logits = _FHE_LLM_topk_sort(module, eval_key, &logits, n_active, bsk_prepared, params);

            // Phase 3: Evaluate ALL expert FFNs (privacy-preserving: cannot skip)
            let expert_outputs: Vec<Vec<GLWE<Vec<u8>>>> = weights
                .expert_ffn_weights
                .iter()
                .map(|ffn_w| _FHE_LLM_ffn(module, eval_key, x, ffn_w, ffn_config))
                .collect();
            ...
            for dim in 0..d_out {
                let mut acc = _FHE_LLM_mul_const(module, &expert_outputs[0][dim], &[weight_scaled]);
                for expert_idx in 1..n_active.min(n_experts) {
                    let term = _FHE_LLM_mul_const(module, &expert_outputs[expert_idx][dim], &[weight_scaled]);
...
///
/// This replaces the BDD circuit estimate with the actual FHE_LLM approach:
/// pairwise subtraction + sign-extraction bootstrapping + conditional swap
/// via ct×ct multiply.
pub fn plan_moe_routing_FHE_LLM(config: &MoEConfig, _params: &FHE_LLMParams) -> MoERoutingPlan {
...
mod tests {
    ...
    #[test]
    fn test_plan_moe_routing_FHE_LLM() {
        let params = FHE_LLMParams::new(crate::params::SecurityLevel::Bits80, crate::params::Precision::Int8);
        let config = MoEConfig {
            n_experts: 8,
            n_active: 2,
            strategy: RoutingStrategy::EncryptedRouting,
        ...
        let plan = plan_moe_routing_FHE_LLM(&config, &params);
    ...
    #[test]
    fn test__FHE_LLM_router_logits() {
        ...
        let logits = _FHE_LLM_router_logits(&module, &ct, &router_weights);
        assert_eq!(logits.len(), 2);

        // Decrypt and verify
        let pt0 = _FHE_LLM_decrypt(&module, &key, &logits[0], &params);
        ...
        let pt1 = _FHE_LLM_decrypt(&module, &key, &logits[1], &params);
    ...
    #[test]
    fn test__FHE_LLM_sign_extract() {
        use crate::bootstrapping::FHE_LLMBootstrapKey;
        use crate::encoding::encode_int8;
        use crate::encrypt::{_FHE_LLM_encrypt, FHE_LLMKey};
        use crate::params::{Precision, SecurityLevel};
        ...
        let ct_pos = _FHE_LLM_encrypt(&module, &key, &pt_pos, [1u8; 32], [2u8; 32]);

        let sign_pos = _FHE_LLM_sign_extract(&module, &ct_pos, &bsk_prepared, &params);
        ...
        let ct_neg = _FHE_LLM_encrypt(&module, &key, &pt_neg, [3u8; 32], [4u8; 32]);

        let sign_neg = _FHE_LLM_sign_extract(&module, &ct_neg, &bsk_prepared, &params);
    ...
    fn test__FHE_LLM_compare() {
        ...
        let cmp = _FHE_LLM_compare(&module, &ct_a, &ct_b, &bsk_prepared, &params);
