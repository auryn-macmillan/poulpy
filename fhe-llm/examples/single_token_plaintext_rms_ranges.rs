...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("exact/plain ranges:");
    for row in pipeline.diagnose_plaintext_rms_ranges_for_token(token_id) {
        println!("  {} mean_sq={:.6}", row.stage, row.mean_sq);
    }
    println!("refreshed/plain ranges:");
    for row in pipeline.diagnose_refreshed_plaintext_rms_ranges_for_token(token_id) {
        println!("  {} mean_sq={:.6}", row.stage, row.mean_sq);
    }
