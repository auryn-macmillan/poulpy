...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let residual_reencode_precisions = parse_u32s(args.next(), &[8]);
    let shift_bits = parse_usizes(args.next(), &[0, 1, 2, 4]);
    let remap_shifts = parse_usizes(args.next(), &[0, 1, 2, 4, 8, 10]);
    let square_res_offsets = parse_usizes(args.next(), &[2, 4, 6, 8, 10, 12, 18]);
    ...
    let sweep = pipeline.sweep_prompt_pre_ffn_norm_advanced(
        &prompt,
        &attn_precisions,
        &residual_precisions,
        &residual_reencode_precisions,
        &shift_bits,
        &remap_shifts,
        &square_res_offsets,
    )?;
    ...
    for row in sweep.rows {
        println!(
            "variant={} attn_decode={} residual_decode={} residual_reencode={} shift={} remap={:?} square_res_offset={:?}: pre_ffn(L-inf={:.3} L2={:.3} MAE={:.3})",
            row.norm_variant,
            row.attn_out_decode_precision,
            row.residual_refresh_decode_precision,
            row.residual_reencode_precision,
            row.input_scale_shift_bits,
            row.remap_shift_bits,
            row.square_res_offset,
            row.pre_ffn_linf,
            row.pre_ffn_l2,
            row.pre_ffn_mae,
        );
